...
const CATALOG_OUT = join(ROOT, "src/data/qaskillsCatalog.ts");
...
function categoryFromPath(kind) {
...
function listSkills(repoRoot, lang) {
  const results = [];
  for (const kind of ["testing-types", "testing-workflows", "skill-engineering"]) {
...
function parseCatalog(readme) {
  ...
  for (const line of lines) {
    ...
    const heading = /^(#{3,5})\s+(.+?)\s*$/.exec(line);
    if (heading) {
      const level = heading[1].length;
      const title = heading[2];
      if (level === 3 && /四层能力|Four capability/i.test(title)) {
        capabilityMap = true;
        root = null;
        subgroup = null;
      } else if (level === 3) {
        capabilityMap = false;
        root = { key: catalogKey(title), title, groups: [], slugs: [] };
        groups.push(root);
        subgroup = null;
      } else if (level === 4 && capabilityMap) {
        root = { key: catalogKey(title), title, groups: [], slugs: [] };
        groups.push(root);
        subgroup = null;
      } else if ((level === 4 && root) || (level === 5 && root)) {
        subgroup = { key: catalogKey(title), title, slugs: [] };
        root.groups.push(subgroup);
      }
      continue;
    }

    const slug = /data-skill:([a-z0-9-]+)/.exec(line)?.[1]
      || /\[`([a-z0-9-]+)`\]\(skills\//.exec(line)?.[1];
    if (!slug || !root) continue;
    if (subgroup) subgroup.slugs.push(slug);
    else root.slugs.push(slug);
  }
  return groups;
}

function buildCatalogModule(zhGroups, enGroups) {
  const groups = zhGroups.map((zhGroup, index) => {
    // README.md and README_EN.md deliberately share the same catalog structure.
    const enGroup = enGroups[index];
    return {
      key: zhGroup.key,
      title: { "zh-cn": zhGroup.title, en: enGroup?.title || zhGroup.title },
      slugs: zhGroup.slugs,
      groups: zhGroup.groups.map((zhSubgroup, subgroupIndex) => {
        const enSubgroup = enGroup?.groups[subgroupIndex];
        return {
          key: zhSubgroup.key,
          title: { "zh-cn": zhSubgroup.title, en: enSubgroup?.title || zhSubgroup.title },
          slugs: zhSubgroup.slugs,
        };
      }),
    };
  });
  return `// Generated by scripts/sync-qaskills-from-repo.mjs. Do not edit manually.\nexport type QASkillCatalogGroup = {\n  key: string;\n  title: { \"zh-cn\": string; en: string };\n  slugs: string[];\n  groups: Array<{ key: string; title: { \"zh-cn\": string; en: string }; slugs: string[] }>;\n};\n\nexport const QA_SKILL_CATALOG: QASkillCatalogGroup[] = ${JSON.stringify(groups, null, 2)};\n`;
}
...
function syncLang(repoRoot, repoLang, siteKey, syncedAt) {
  ...
  for (const skill of skills) {
    ...
    const category = categoryFromPath(skill.kind);
...
function main() {
  ...
  try {
    ...
    const zhReadme = readFileSync(join(repo.path, "README.md"), "utf8");
    const enReadme = readFileSync(join(repo.path, "README_EN.md"), "utf8");
    writeFileSync(CATALOG_OUT, buildCatalogModule(parseCatalog(zhReadme), parseCatalog(enReadme)), "utf8");
