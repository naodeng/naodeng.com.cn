...
1. **一级标题**：全文仅一个 `# 中文名称 (English Name)` 或 `# 中文 ｜ English`，且**中文在前、英文在后**。
2. **目录 (TOC)**：在 `## 关于xxx的问题？` 下可包含 `## 目录` 及锚点列表，便于跳转到各 `####` 小节。运行 `node scripts/wiki-toc-and-format.mjs` 可为无 TOC 词条自动生成。
3. **二级及以下标题**：统一使用「中文 (English)」格式，便于扫读。
4. **段落与列表**：标题前后各留空行，列表项之间可适当空行，代码块前后留空行；文件末尾至多保留一个换行，避免多处连续空行。
