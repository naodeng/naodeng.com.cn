...
- [x] 确认上线节奏：一次性发布。
- [x] T0.2 建立指标基线（上线前 28 天：CTR/滚动深度/二跳率/Lighthouse）。
...
### Outputs
- `docs/archive/ui-redesign/ui-redesign-prd-v2.md`（冻结版本）
- `docs/archive/ui-redesign/ui-redesign-design-v2.md`（冻结版本）
- 基线数据记录（文档或表格链接）
...
- [x] 设计规范速查表（可附在 `docs/archive/ui-redesign/`）。

### Acceptance
- [x] 基础组件在 Blog/Wiki/AIWiki/Guild 外观一致。
- [x] 无明显颜色与间距漂移。
- [x] 键盘焦点可见，且不依赖鼠标悬停状态。
...
### Acceptance
- [x] 三类频道可互相导流，二跳路径清晰。
- [x] 侧栏、目录、正文三栏在桌面端布局稳定。
- [x] 广告位数量与位置满足密度规则，且不破坏阅读节奏。
- [x] 广告不插入 H1 与首段正文之间，正文主链路连续。
- [x] 广告渲染不引起明显 CLS 波动（占位生效）。
- [x] 例外页可按策略不展示广告位且不影响布局。
...
## 依赖与阻塞项
- [x] SEO 改造项与 UI 改造项冲突排期。
- [x] 内容团队确认中英文文案长度边界。
- [x] 广告位实现与 CLS 监控联调（防止影响体验与稳定性）。
...
- `docs/archive/wiki/related-terms-rule-v2.md`
