---
title: "Cypress：安装与第一个端到端测试"
description: "在 JavaScript 项目中接入 Cypress，配置基础地址并运行第一个浏览器测试。"
testType: ui-testing
framework: cypress
...
Cypress：面向开发者的浏览器测试工具，提供交互式命令日志、查询与断言的自动重试，以及一等的网络拦截能力。当测试套件能控制状态并让每个 spec 独立时，它特别适合对运行中 Web 应用做端到端测试。 本文把这些能力落到“第一个资料保存端到端 spec”这个可执行任务上。先建立最小端到端场景：确定目标、一个用户结果，以及另一位工程师无需依赖你本机环境即可运行的命令。本文刻意保持范围很小。一个通过的小测试先证明工具、浏览器或施压机与目标可以通信，再增加数据变化、并发、重试或报告。请从已授权、可恢复的目标开始，例如本地测试应用或预发布环境；在运行前记录目标版本、测试数据来源和结果负责人。这样可以避免把“工具运行成功”误读成“允许发布”。
...
把场景保存为有名字的文件，把数据放进独立 fixture 或 CSV，并通过环境变量传入目标。本阶段应使用工具原生的等待、事务、拦截、报告或配置能力来处理 getting-started 关注点，不能用通用 sleep 或隐藏的全局默认值敷衍。以下配置和命令聚焦于“第一个资料保存端到端 spec”，也适合作为提交前的最小冒烟检查。
