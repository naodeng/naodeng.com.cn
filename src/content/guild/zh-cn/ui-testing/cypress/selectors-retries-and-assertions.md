---
title: "Cypress：选择器、重试机制与断言"
description: "使用稳定选择器和 Cypress 自动重试，表达可靠的用户可见预期。"
testType: ui-testing
framework: cypress
...
Cypress：面向开发者的浏览器测试工具，提供交互式命令日志、查询与断言的自动重试，以及一等的网络拦截能力。当测试套件能控制状态并让每个 spec 独立时，它特别适合对运行中 Web 应用做端到端测试。 本文把这些能力落到“稳定 data-cy 定位与可重试断言”这个可执行任务上。把只走通 happy path 的示例变成明确契约。目标不是把脚本写得更长，而是让输入、同步和通过标准清晰可见。参数化会变化的值，断言用户真正依赖的响应或 UI 状态，并提供能让评审者定位契约断点的失败信息。请从已授权、可恢复的目标开始，例如本地测试应用或预发布环境；在运行前记录目标版本、测试数据来源和结果负责人。这样可以避免把“工具运行成功”误读成“允许发布”。
...
把场景保存为有名字的文件，把数据放进独立 fixture 或 CSV，并通过环境变量传入目标。本阶段应使用工具原生的等待、事务、拦截、报告或配置能力来处理 selectors-retries-and-assertions 关注点，不能用通用 sleep 或隐藏的全局默认值敷衍。以下配置和命令聚焦于“稳定 data-cy 定位与可重试断言”，也适合作为提交前的最小冒烟检查。
