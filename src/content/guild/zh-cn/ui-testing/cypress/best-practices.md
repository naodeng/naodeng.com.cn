---
title: "Cypress UI 自动化最佳实践"
description: "通过确定性状态、可观察断言和有纪律的重试，维护可持续的 Cypress 测试。"
testType: ui-testing
framework: cypress
...
Cypress：面向开发者的浏览器测试工具，提供交互式命令日志、查询与断言的自动重试，以及一等的网络拦截能力。当测试套件能控制状态并让每个 spec 独立时，它特别适合对运行中 Web 应用做端到端测试。 本文把这些能力落到“测试产品行为的确定性套件”这个可执行任务上。采用可重复的评审循环：先定义业务风险，选择能暴露风险的最低测试层，让数据和环境可观测，并保留能够解释失败的产物。下面示例是一份检查清单，可在把绿色运行当作发布决策证据之前使用。请从已授权、可恢复的目标开始，例如本地测试应用或预发布环境；在运行前记录目标版本、测试数据来源和结果负责人。这样可以避免把“工具运行成功”误读成“允许发布”。
...
把场景保存为有名字的文件，把数据放进独立 fixture 或 CSV，并通过环境变量传入目标。本阶段应使用工具原生的等待、事务、拦截、报告或配置能力来处理 best-practices 关注点，不能用通用 sleep 或隐藏的全局默认值敷衍。以下配置和命令聚焦于“测试产品行为的确定性套件”，也适合作为提交前的最小冒烟检查。
