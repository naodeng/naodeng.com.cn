---
title: "Playwright UI 自动化最佳实践"
description: "通过语义定位器、隔离状态和证据驱动排障，让 Playwright 套件保持快速可信。"
testType: ui-testing
...
把场景保存为有名字的文件，把数据放进独立 fixture 或 CSV，并通过环境变量传入目标。本阶段应使用工具原生的等待、事务、拦截、报告或配置能力来处理 best-practices 关注点，不能用通用 sleep 或隐藏的全局默认值敷衍。以下配置和命令聚焦于“快速隔离并保留发布证据的套件”，也适合作为提交前的最小冒烟检查。
