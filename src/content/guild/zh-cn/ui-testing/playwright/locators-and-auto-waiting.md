---
title: "Playwright：定位器、断言与自动等待"
description: "使用面向用户的定位器和 Web-first 断言，编写稳定的 Playwright 测试。"
testType: ui-testing
...
把场景保存为有名字的文件，把数据放进独立 fixture 或 CSV，并通过环境变量传入目标。本阶段应使用工具原生的等待、事务、拦截、报告或配置能力来处理 locators-and-auto-waiting 关注点，不能用通用 sleep 或隐藏的全局默认值敷衍。以下配置和命令聚焦于“语义定位器与 Web-first 断言”，也适合作为提交前的最小冒烟检查。
