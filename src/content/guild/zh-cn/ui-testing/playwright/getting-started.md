---
title: "Playwright：安装、配置与第一个端到端测试"
description: "初始化 Playwright Test、安装浏览器，并运行一个小型跨浏览器 UI 测试。"
testType: ui-testing
...
把场景保存为有名字的文件，把数据放进独立 fixture 或 CSV，并通过环境变量传入目标。本阶段应使用工具原生的等待、事务、拦截、报告或配置能力来处理 getting-started 关注点，不能用通用 sleep 或隐藏的全局默认值敷衍。以下配置和命令聚焦于“跨浏览器的资料保存测试”，也适合作为提交前的最小冒烟检查。
