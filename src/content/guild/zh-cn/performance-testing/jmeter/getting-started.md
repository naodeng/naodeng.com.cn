---
title: "JMeter 性能测试：安装与第一个测试计划"
description: "安装 JMeter，创建最小 HTTP 测试计划，并理解 GUI 与 CLI 的职责边界。"
...
把场景保存为有名字的文件，把数据放进独立 fixture 或 CSV，并通过环境变量传入目标。本阶段应使用工具原生的等待、事务、拦截、报告或配置能力来处理 getting-started 关注点，不能用通用 sleep 或隐藏的全局默认值敷衍。以下配置和命令聚焦于“五用户健康检查测试计划”，也适合作为提交前的最小冒烟检查。
