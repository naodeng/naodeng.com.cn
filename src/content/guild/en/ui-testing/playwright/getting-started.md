---
title: "Playwright: Install, Configure, and Run Your First E2E Test"
description: "Initialize Playwright Test, install browsers, and run a small cross-browser UI test."
testType: ui-testing
...
Playwright Test is an end-to-end framework that bundles a runner, web-first assertions, isolated browser contexts, parallel execution, and rich diagnostics. It supports Chromium, Firefox, and WebKit with one API, making it suitable for modern web flows that need fast local feedback and CI evidence. This guide turns that capability into a cross-browser saved-profile test. Build the smallest end-to-end scenario first: a known target, one user outcome, and one command that another engineer can run without your machine-specific assumptions. This article is deliberately narrow. A small passing test proves that the tool, browser or injector, and target can communicate before you add data variation, concurrency, retries, or reporting. Start from an authorized, disposable target such as a staging environment or a local test application. Record the target version, test data source, and owner of the result before running anything. That context prevents a technically successful run from being misread as release approval.
...
// Run the focused getting-started scenario.
// npx playwright test tests/getting_started.spec.ts --project=chromium
```
