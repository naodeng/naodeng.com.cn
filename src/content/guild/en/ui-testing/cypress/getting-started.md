---
title: "Cypress: Install and Write Your First End-to-End Test"
description: "Add Cypress to a JavaScript project, configure a base URL, and run a first browser test."
testType: ui-testing
framework: cypress
...
duration: 35
order: 1
officialDocs: https://docs.cypress.io/app/get-started/install-cypress
---

## Concept and goal

Cypress is a developer-oriented browser-testing tool with an interactive command log, automatic retries for queries and assertions, and first-class network interception. It is especially productive for end-to-end tests of a running web application when the suite controls state and keeps each spec independent. This guide turns that capability into a first end-to-end saved-profile spec. Build the smallest end-to-end scenario first: a known target, one user outcome, and one command that another engineer can run without your machine-specific assumptions. This article is deliberately narrow. A small passing test proves that the tool, browser or injector, and target can communicate before you add data variation, concurrency, retries, or reporting. Start from an authorized, disposable target such as a staging environment or a local test application. Record the target version, test data source, and owner of the result before running anything. That context prevents a technically successful run from being misread as release approval.
...
// npx cypress run --spec cypress/e2e/getting_started.cy.js --browser chrome
```
