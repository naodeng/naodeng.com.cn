...
lang: "zh-cn"
summary: "允许模型在受控流程中选择并调用带类型参数的工具函数。"
...
> 一句话定义：允许模型在受控流程中选择并调用带类型参数的工具函数。
...
`函数调用` 常被误解为“高级功能”，但它本质上是为了解决工程交付中的基础问题：结果不稳定、流程不可复用、问题难以追踪。从科普视角看，它的价值在于把 AI 从“会回答”推进到“可落地”。

## 核心定义

### 标准定义

允许模型在受控流程中选择并调用带类型参数的工具函数。

### 通俗解释

如果把 AI 工作流比作流水线，`函数调用` 就是其中负责“减少出错、提高可复用性”的关键工位。它不是为了炫技，而是为了让团队在真实项目里更稳地交付结果。
...
| 维度 | 函数调用 | 结构化输出 | 钩子机制 |
| :--- | :--- | :--- | :--- |
| 核心目标 | 聚焦 `函数调用` 的核心能力与边界 | 更偏向 `结构化输出` 的能力侧重 | 更偏向 `钩子机制` 的能力侧重 |
