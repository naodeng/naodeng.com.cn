...
slug: "hooks"
lang: "zh-cn"
summary: "在智能体/工具生命周期事件前后注册回调，用于策略控制与自动化处理。"
...
> 一句话定义：在智能体/工具生命周期事件前后注册回调，用于策略控制与自动化处理。
...
`钩子机制` 常被误解为“高级功能”，但它本质上是为了解决工程交付中的基础问题：结果不稳定、流程不可复用、问题难以追踪。从科普视角看，它的价值在于把 AI 从“会回答”推进到“可落地”。

## 核心定义

### 标准定义

在智能体/工具生命周期事件前后注册回调，用于策略控制与自动化处理。

### 通俗解释

如果把 AI 工作流比作流水线，`钩子机制` 就是其中负责“减少出错、提高可复用性”的关键工位。它不是为了炫技，而是为了让团队在真实项目里更稳地交付结果。
...
| 维度 | 钩子机制 | 钩子事件 | 函数调用 |
| :--- | :--- | :--- | :--- |
| 核心目标 | 聚焦 `钩子机制` 的核心能力与边界 | 更偏向 `钩子事件` 的能力侧重 | 更偏向 `函数调用` 的能力侧重 |
...
- [hook-events](https://inaodeng.com/zh-cn/AIWiki/hook-events/)
- [function-calling](https://inaodeng.com/zh-cn/AIWiki/function-calling/)
- [custom-commands](https://inaodeng.com/zh-cn/AIWiki/custom-commands/)
