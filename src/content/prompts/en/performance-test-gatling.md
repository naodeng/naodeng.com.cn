...
5. Loose verbal goals (--úsurvive the sale--ù, --úAPIs must not be slow--ù)
...
Extract only paths, methods, load numbers, and thresholds that **actually appear** in the materials. Put gaps in --úOpen Questions--ù; do not invent a full fake SLA.
...
| --- | --- | --- |
| Baseline | First profiling, before/after change, no history yet | --úEstablish latency waterline for an API/flow--ù |
| Load | Validate target concurrency / throughput | --úHold N users / M RPS steadily--ù |
| Stress | Find capacity cliff or degradation point | --úHow far until it melts--ù |
| Spike | Campaign / flash-sale / burst risk | --úSurvive a sudden surge then recover--ù |
| Soak (endurance) | Pre-release stability, leak suspicion | --úHold for hours without drift--ù |

Decision rules:

1. User only says --údo performance testing--ù with no more signal -Üí **default to load** (or --úbaseline + short load--ù), and explain why stress/spike/soak are deferred.
2. Clear peak target -Üí load first; add stress only if the user cares about --úceiling / degrade--ù.
3. Promo / flash-sale / burst language -Üí load + spike (or spike alone if a daily-load baseline already exists).
4. Leak / long-run / overnight gate -Üí soak; do not use soak as a substitute for first profiling.
5. Combine types only when the user asks; order by risk (usually: baseline -Üí load -Üí spike/stress -Üí soak).
...
Mirror k6--ôs `http_req_duration` p95 / `http_req_failed` with a Gatling global assertion skeleton:
...
- End with an Open Questions list (see Output ¬ß6) covering at least: peak traffic, target p95/error rate, safe-to-load env, feeder data and auth
...
- Do not ship a --ústandard pack--ù that enables all five scenario types by default.
...
- P0 transactions: method + path (confirmed) or --úpath TBD--ù
...
- Suggested order (tiny-user smoke -Üí chosen scenario -Üí optional push)
...
- [ ] P0 transactions and injection model are concrete and actionable --- not --úcare about performance--ù fluff
