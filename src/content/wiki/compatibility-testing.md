...
- **[数据库](/zh-cn/wiki/database/) 版本**
    和配置

- **其他软件应用程序**
    （例如防病毒程序或第三方插件）
  [兼容性测试](/zh-cn/wiki/compatibility-testing/) 可以是手动的，也可以是自动化的，可以使用模拟不同环境和配置的工具进行自动化测试。确保软件为所有用户提供一致的体验至关重要，无论他们的系统是什么[环境搭建](/zh-cn/wiki/setup/)。
...
- **[回归测试](/zh-cn/wiki/regression-testing/)**
  ...
  [兼容性测试](/zh-cn/wiki/compatibility-testing/) 的独特之处在于它注重软件在不同环境中运行的能力，其中包括不同的操作系统、浏览器版本、网络环境和硬件配置。它确保软件提供一致的用户体验，无论用户的技术 [环境搭建](/zh-cn/wiki/setup/) 有何变化。其他类型的测试可能无法专门解决这些环境变化。
...
- **旧系统**：验证新版本的企业软件是否仍可与旧版 [数据库](/zh-cn/wiki/database/) 或某些客户端使用的旧硬件一起使用。
