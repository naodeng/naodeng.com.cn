...
常用工具包括 **Mockito**、**Moq**、**Sinon.js** 和 **[Jest](/zh-cn/wiki/jest/)**。
  对于[database](/zh-cn/wiki/database/)，您可以模拟数据访问层或存储库，设置预期的查询及其结果：
...
- 精确的期望配置以避免误报/漏报。
- 测试之间的模拟状态的清理和重置。
  通过关注基本交互并使用工厂方法进行模拟创建，可以缓解过度模拟或维护复杂模拟[setups](/zh-cn/wiki/setup/) 等挑战。
  为了确保有效性，请定期审查和重构模拟测试，以符合当前的系统行为和要求。
  通过将模拟测试包含在每个构建或部署上运行的 [测试套件](/zh-cn/wiki/test-suite/) 中，将模拟测试集成到 CI/CD 中。
  在 TDD 中，[模拟测试](/zh-cn/wiki/mock-testing/) 用于在实现实际组件之前测试驱动接口和交互的设计。
...
[模拟测试](/zh-cn/wiki/mock-testing/) 在软件开发中对于隔离被测系统至关重要，确保测试不受外部依赖项或有状态组件的影响。通过使用模拟对象，开发人员可以模拟复杂的真实系统的行为，这些系统可能无法或难以配置用于测试目的。这种隔离有助于查明正在测试的代码单元内的缺陷，从而产生更可靠和可维护的代码。
  ...
  模拟还支持**行为[verification](/zh-cn/wiki/verification/)**，确保被测系统以预期的方式与其依赖项进行交互。这在**面向服务的架构**中尤其重要，其中组件之间的交互至关重要。
...
- **速度**：使用模拟的测试运行速度更快，因为它们避免了设置实际依赖项并与实际依赖项交互的开销，例如 [databases](/zh-cn/wiki/database/) 或 Web 服务。
- **确定性**：模拟提供确定性行为，确保测试每次运行时都会产生相同的结果，而对于可能具有可变状态的真实依赖项而言，情况并非总是如此。
- **焦点**：通过使用模拟，测试仅关注代码的逻辑，而不是与其他系统的集成，这是集成测试所涵盖的。
  以下是使用 [Jest](/zh-cn/wiki/jest/) 在 TypeScript 中创建模拟的示例：
...
- **行为[Verification](/zh-cn/wiki/verification/)**：使用模拟的测试通常侧重于验证被测系统是否以预期的方式与模拟进行交互，例如使用正确的参数调用方法。
...
- **关注工作单元**：[模拟测试](/zh-cn/wiki/mock-testing/) 鼓励通过单独测试来关注工作单元，从而促进更好的设计和更可维护的代码。
  请记住，[模拟测试](/zh-cn/wiki/mock-testing/) 应与其他测试方法结合使用以确保全面覆盖，因为它不测试与真实依赖项的集成。
...
2. **集成模拟测试**
  ...
  通过遵循这些步骤，[模拟测试](/zh-cn/wiki/mock-testing/) 成为开发周期的无缝部分，从而可以及早发现问题并持续验证系统行为，而不受外部依赖的影响。
...
1. **确定要用模拟替换的依赖项**。这可以是外部服务、[database](/zh-cn/wiki/database/) 或您的系统与之交互的任何其他组件。
...
要为 [database](/zh-cn/wiki/database/) 创建模拟测试，请按照以下步骤操作：

1. **识别您的应用程序执行的需要测试的[database](/zh-cn/wiki/database/) 操作**。
2. **选择与您的测试环境兼容的模拟框架**，例如用于 Java 的 Mockito 或用于 .NET 的 Moq。
3. **创建一个模拟[database](/zh-cn/wiki/database/) 接口**，代表实际的[database](/zh-cn/wiki/database/) 操作。此接口应模仿真实[database](/zh-cn/wiki/database/) 服务的行为。
...
5. **将模拟对象**注入到被测系统中，替换真正的[database](/zh-cn/wiki/database/)依赖项。
6. **使用模拟对象编写[测试用例](/zh-cn/wiki/test-case/)**，以通过受控、可预测的[database](/zh-cn/wiki/database/) 交互来验证系统的行为。
...
7. **运行测试**以确保它们通过模拟[database](/zh-cn/wiki/database/)。根据需要调整模拟的行为以覆盖不同的场景。
  通过将系统与真实的[database](/zh-cn/wiki/database/)隔离，您可以测试各种数据条件和错误情况，而无需依赖实际的[database](/zh-cn/wiki/database/)，从而实现更快、更可靠的测试。

1. **识别您的应用程序执行的需要测试的[database](/zh-cn/wiki/database/) 操作**。
2. **选择与您的测试环境兼容的模拟框架**，例如用于 Java 的 Mockito 或用于 .NET 的 Moq。
3. **创建一个模拟[database](/zh-cn/wiki/database/) 接口**，代表实际的[database](/zh-cn/wiki/database/) 操作。该接口应该模仿真实[database](/zh-cn/wiki/database/) 服务的行为。
...
- **平衡模拟与端到端测试**：用端到端测试补充模拟测试，以确保系统在类似生产的环境中按预期工作。
  通过实施这些策略，[测试自动化](/zh-cn/wiki/test-automation/) 工程师可以缓解与[模拟测试](/zh-cn/wiki/mock-testing/) 相关的挑战，并提高[测试套件](/zh-cn/wiki/test-suite/) 的可靠性和有效性。
...
- **不隔离测试**：如果模拟 [setup](/zh-cn/wiki/setup/) 或状态在测试之间共享，则可能导致测试间依赖和不可预测的结果。隔离每个[测试用例](/zh-cn/wiki/test-case/)以确保它们独立运行。
...
- **验证交互**：使用[verification](/zh-cn/wiki/verification/)方法来检查被测系统是否按预期与模拟进行交互。
...
- **同行评审**：让同行评审模拟测试，以发现原作者可能忽略的问题。
  通过遵循这些准则，您可以提高模拟测试的有效性，并为更加强大和可靠的 [软件测试](/zh-cn/wiki/software-testing/) 流程做出贡献。
...
在此示例中，每当在 `MyService` 内调用 `dependencyMethod` 时，它都会被存根以返回 `'stubbed value'`。这允许测试验证行为而不依赖于 `dependencyMethod` 的实际实现。
  在处理**外部服务**、**[database](/zh-cn/wiki/database/) 调用**或任何其他会在测试中引入复杂性或不确定性的组件时，存根特别有用。它有助于创建 **隔离** 和 **可预测** [测试环境](/zh-cn/wiki/test-environment/)，其中重点是正在测试的代码单元。
...
- **控制[测试环境](/zh-cn/wiki/test-environment/)**
...
- **压力测试**
  ...
  以下是在性能 [测试场景](/zh-cn/wiki/test-scenario/) 中为服务创建模拟的示例：
