...
- **可扩展性测试的关键组成部分：**
  ...
  - **监控系统**：实时观察系统行为的仪表盘。
  - **[测试环境](/zh-cn/wiki/test-environment/)**：尽可能模拟生产环境。
  - **可扩展性模型**：用于预测未来性能。
...
- **使用了哪些不同的技术？**
  ...
  - **基于模型的测试**。
  - **[耐久性测试 (Endurance Testing)](/zh-cn/wiki/endurance-testing/)**：检查内存泄漏。
  - **[容量测试 (Volume Testing)](/zh-cn/wiki/volume-testing/)**：增加数据库数据量。
  - **峰值测试 (Spike Testing)**：突然大量负载。
  - **隔离测试**：独立扩展特定服务。
  - **基于云的可扩展性测试**。

- **常用工具：**
  - **[JMeter](/zh-cn/wiki/jmeter/)**：开源 Web 应用负载测试。
