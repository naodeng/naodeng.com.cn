...
- **生成**
      屏幕截图以查看不同设备和浏览器的布局。
  测试后，**查看**结果，其中可能包括视频记录、日志和屏幕截图，具体取决于所执行的测试类型。 **分析**结果以识别任何问题或[bugs](/zh-cn/wiki/bug/)。
...
1. 通过以下方式监控测试结果
    **[browserstack](/zh-cn/wiki/browserstack/) 仪表板**
    您可以在其中查看视频记录、日志和屏幕截图。
  请记住将 `"YOUR_USERNAME"` 和 `"YOUR_ACCESS_KEY"` 替换为您的实际 [browserstack](/zh-cn/wiki/browserstack/) 凭据。

1. **登录**您的[browserstack](/zh-cn/wiki/browserstack/) 帐户。
2. 导航到 **App Live** 或 **App Automate** 部分，具体取决于您要执行手动操作还是 [自动化测试](/zh-cn/wiki/automated-testing/)。
3. 对于**应用直播**：
- 上传您的移动应用程序或提供公共 URL。
- 选择您要测试的设备和操作系统版本。
- 通过浏览器与所选设备上的应用程序交互。
...
1. 安装
    **[browserstack](/zh-cn/wiki/browserstack/) Jenkins 插件**
    来自 Jenkins 插件管理器。
...
3. 在作业配置中，添加构建步骤或构建后操作
    **在[browserstack](/zh-cn/wiki/browserstack/)上运行自动化测试**
    。
...
6. 启动 Jenkins 构建，插件将自动触发 BrowserStack 上的测试。
  以下是如何配置 Jenkins 作业以使用 [browserstack](/zh-cn/wiki/browserstack/) 的示例：
...
- **Visual Studio Team Services**：将 VSTS 管道与 [browserstack](/zh-cn/wiki/browserstack/) 连接，以在发布过程中运行自动化测试。
  这些集成有助于利用 [browserstack](/zh-cn/wiki/browserstack/) 的设备和浏览器覆盖范围，使其成为全面 [自动化测试](/zh-cn/wiki/automated-testing/) 的多功能选择。
...
- **企业功能**：为大型组织定制的解决方案，包括单点登录 (SSO)、团队管理和[priority](/zh-cn/wiki/priority/) 支持。
  为了利用这些功能，工程师可以使用提供的[API](/zh-cn/wiki/api/)和CLI工具将相关的[browserstack](/zh-cn/wiki/browserstack/)功能合并到现有的[测试自动化](/zh-cn/wiki/test-automation/)框架中。例如，要启用本地测试，请使用 BrowserStackLocal 二进制文件：
...
5. 单击
  ...
  请记住，实时测试适用于[手动测试](/zh-cn/wiki/manual-testing/)。对于[自动化测试](/zh-cn/wiki/automated-testing/)，您将使用[browserstack](/zh-cn/wiki/browserstack/) 的自动化功能或其他[自动化测试](/zh-cn/wiki/automated-testing/) 集成。
...
9. **整合**
  ...
  对于您的应用程序的 **[自动化测试](/zh-cn/wiki/automated-testing/)**，请切换到 [browserstack](/zh-cn/wiki/browserstack/) 中的 **自动化** 部分，并按照您选择的框架（例如 Appium 或 Espresso）的相关步骤进行操作。
  请记住，App Live 用于**手动交互测试**。对于自动化测试，请使用[browserstack](/zh-cn/wiki/browserstack/) 的 **Automate** 或 **Automate Pro** 功能。
