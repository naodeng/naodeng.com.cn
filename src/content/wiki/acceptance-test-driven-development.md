...
- [TDD 测试驱动开发](/zh-cn/wiki/test-driven-development/)
...
用于 ATDD 的常用工具包括 Cucumber、SpecFlow 和 FitNesse，它们支持行为驱动开发（[BDD](/zh-cn/wiki/bdd/)）和以实例为基础的规范实践。这些工具允许使用所有利益相关者都能理解的语言编写测试，弥合了技术和非技术团队成员之间的差距。
...
此外，ATDD 对自动化的强调支持[回归测试](/zh-cn/wiki/regression-testing/)，实现了一个可处理变化而不需要显著增加工作量的可持续测试过程。这个[自动化测试](/zh-cn/wiki/automated-testing/)框架对于在快节奏的开发环境中保持高质量尤为重要，特别是在扩大到较大项目时。
...
ATDD 中的这种前期协作确保所有方对需求和“完成”的定义有共同的理解。它将重点从开发后发现[缺陷](/zh-cn/wiki/bug/)转移到通过早期澄清期望来预防[缺陷](/zh-cn/wiki/bug/)。此外，ATDD 鼓励**行为驱动开发**（[BDD](/zh-cn/wiki/bdd/)），其中测试以所有利益相关者都能理解的语言编写，通常使用 Given-When-Then 格式。

而传统测试方法可能会严重依赖[手动测试](/zh-cn/wiki/manual-testing/)或在事后创建自动化测试，ATDD 从一开始就集成了**[自动化测试](/zh-cn/wiki/automated-testing/)**。验收测试被自动化，并成为**回归套件**的一部分，提供关于新更改是否符合已达成一致的标准的即时反馈。
...
ATDD 通过确保准确满足**[功能需求](/zh-cn/wiki/functional-requirements/)** 并使产品表现如利益相关者期望的方式，提高了[软件质量](/zh-cn/wiki/software-quality/)。通过从一开始关注客户需求，ATDD 促进了**清晰且可执行的规范**的创建。这些规范指导开发和测试，减少了由于误解或不完整的需求而导致的缺陷的可能性。
...
来自验收测试执行的持续反馈允许**早期检测问题**，通常比在开发周期后期发现的[缺陷](/zh-cn/wiki/bug/)更具成本效益。此外，验收测试套件成为可以用于**[回归测试](/zh-cn/wiki/regression-testing/)**的实时文档，确保新更改不会破坏现有功能。

总的来说，ATDD 通过澄清需求、促进协作和提供持续反馈，有助于构建与业务需求和用户期望密切一致的产品，从而提高[软件质量](/zh-cn/wiki/software-quality/)。
...
6. **[测试执行](/zh-cn/wiki/test-execution/)**：验证软件是否符合达成一致的验收标准。
7. **审查和反馈：** 利益相关者审查验收测试，确认其覆盖所需的功能和行为。
8. **[迭代](/zh-cn/wiki/iteration/)**：根据需要通过这些步骤进行迭代，直到功能符合验收标准。

验收测试通常是自动化的，以便频繁执行和[回归测试](/zh-cn/wiki/regression-testing/)。测试以对所有相关方可理解的语言编写，通常使用行为驱动开发（[BDD](/zh-cn/wiki/bdd/)）框架，如 Cucumber 或 SpecFlow。这确保了测试既充当规范又用于验证。
...
- **行为驱动开发（[BDD](/zh-cn/wiki/bdd/)）**：这一技术涉及以自然语言风格编写测试，描述应用程序的行为。通常使用诸如 Cucumber 或 SpecFlow 之类的工具来促进[BDD](/zh-cn/wiki/bdd/)。
...
- **协作工具**：使用促进业务利益相关者、开发人员和测试人员之间协作的工具，例如共享存储库或像[JIRA](/zh-cn/wiki/jira/)与 Xray 或 TestRail 这样的协作平台。
- **持续集成（CI）**：将验收测试作为 CI 流水线的一部分自动运行，以获取有关所做更改的即时反馈。
- **测试工件的版本控制**：将验收测试与代码库一起存储在版本控制系统中，以保持[测试用例](/zh-cn/wiki/test-case/)与应用程序代码之间的同步。
...
在开发过程中，测试人员参与验收测试的**持续完善**，通常与开发人员搭档创建和维护[自动化测试](/zh-cn/wiki/test-automation/)。他们确保测试是**可执行的规范**，指导开发并即时提供有关软件行为是否符合预期结果的反馈。

测试人员还在**维护[测试套件](/zh-cn/wiki/test-suite/)**方面发挥了关键作用，确保随着代码库的演变它保持**可靠**和**高效**。他们还可能负责**[测试数据](/zh-cn/wiki/test-data/)管理**和设置必要的**[测试环境](/zh-cn/wiki/test-environment/)**。

在 ATDD 循环中，测试人员帮助促进**三位好朋友会议**，讨论特性实施的问题，并积极参与**[迭代](/zh-cn/wiki/iteration/)计划** 和 **回顾**以不断改进流程。
...
- **Serenity [BDD](/zh-cn/wiki/bdd/)**：通过提供先进的报告和实时文档功能，增强了其他 BDD 工具（如 Cucumber 和 JBehave）。

这些工具通过使用所有利益相关者都能理解的语言定义验收标准，支持 ATDD 过程。它们促进了验收测试的自动化，并帮助确保软件特性在被视为完成之前满足预定义的标准。[自动化测试](/zh-cn/wiki/test-automation/)工程师使用这些工具来编写、管理和执行验收测试，通常将它们集成到持续集成流水线中以获得持续反馈。熟练掌握编程，理解应用领域，以及熟悉所选工具的语法和最佳实践对有效使用这些工具至关重要。

### 这些工具如何支持 ATDD 流程？

自动化测试工具通过执行以**行为驱动**格式编写的验收测试，支持**ATDD 过程**。这些工具通常与**Cucumber**、**SpecFlow**或**FitNesse**等框架集成，这些框架允许使用**业务可读**语言（如[Gherkin](/zh-cn/wiki/gherkin/)）定义测试。

通过使用这些工具，团队可以自动化验收标准的**验证**，确保软件符合达成的规范。这种自动化支持**持续集成**（CI）实践，允许在代码提交时自动运行测试，向开发人员提供**即时反馈**。

此外，[自动化测试](/zh-cn/wiki/test-automation/)工具通过维护一套测试，可用于验证更改是否破坏了现有功能，从而支持**重构**。在 ATDD 中，重点是在整个开发过程中满足验收标准，这一点至关重要。
...
要有效使用[自动化测试](/zh-cn/wiki/test-automation/)工具，需要具备以下几项技能：
...
- **版本控制系统**：能够使用 Git 等版本控制系统管理[测试脚本](/zh-cn/wiki/test-script/)并与开发团队协作。
- **持续集成/持续部署（CI/CD）**：了解 CI/CD 管道和 Jenkins、CircleCI 或 Travis CI 等工具，以将自动化测试集成到构建流程中。
- **自动化脚本编写**：具备脚本编写技能，以创建健壮、可维护和可重用的[测试脚本](/zh-cn/wiki/test-script/)。
- **理解 ATDD**：虽然不需要详细涵盖，但理解 ATDD 原则对创建反映用户需求的验收测试至关重要。
- **解决问题和分析技能**：能够解决[测试脚本](/zh-cn/wiki/test-script/)的问题并适应变化的需求或环境。
- **注意细节**：在编写[测试用例](/zh-cn/wiki/test-case/)时要精确，以覆盖边缘情况，防止[误报](/zh-cn/wiki/false-positive/)或漏报。
...
- **性能和[安全测试](/zh-cn/wiki/security-testing/)**：了解性能瓶颈和安全漏洞，将相关测试纳入自动化测试套件。
...
- **增加[测试覆盖率](/zh-cn/wiki/test-coverage/)**，在较短的时间内执行更多的测试。
...
开始时，进行一个**规划会议**，讨论用户故事并创建验收测试。这确保了对功能及其期望结果的共享理解。使用像 Cucumber 或 SpecFlow 这样的**行为驱动开发 ([BDD](/zh-cn/wiki/bdd/)) 工具**编写验收测试，以一种对所有相关方都可理解的语言。
...
- **维护[测试套件](/zh-cn/wiki/test-suite/)**：随着应用程序的演变，保持验收测试的最新状态可能会耗费时间。
- **[测试数据](/zh-cn/wiki/test-data/)管理**：生成和管理验收测试所需的数据可能会很复杂，尤其是在处理多个环境时。
- **在覆盖率和速度之间取得平衡**：在保持测试套件对持续集成足够快的同时，实现足够的测试覆盖率可能很困难。
- **[不稳定的测试](/zh-cn/wiki/flaky-test/)**：间歇性地通过和失败的测试可能会损害对测试套件的信心，并在调查中浪费时间。
...
- 使用**[测试数据](/zh-cn/wiki/test-data/)管理**策略，确保测试具有所需数据且处于正确的状态。
- 根据风险和业务价值**优先考虑**测试，首先关注最关键的功能。
- 培养一个**质量文化**，使每个人都对产品的质量负责，而不仅仅是测试团队。
- 定期**审查和调整**您的 ATDD 流程，以解决任何问题并提高效率。

通过遵循这些实践，您可以增强协作，确保需求的清晰度，并在整个开发生命周期中保持高水平的[软件质量](/zh-cn/wiki/software-quality/)。

### 如何为大型项目扩展 ATDD？

在大型项目中扩展 ATDD 需要战略规划和高效的工具。首先，通过**构建验收测试**来反映项目的模块化架构。这使得可以在不同团队之间并行进行开发和测试。利用**版本控制**管理测试工件，并确保在团队之间同步。

利用**[测试数据](/zh-cn/wiki/test-data/)管理**提供一致且隔离的[测试环境](/zh-cn/wiki/test-environment/)，避免共享数据时可能出现的冲突和依赖关系。实施 **持续集成 CI**以自动运行验收测试来验证新的代码提交，提供对集成状态的即时反馈。

**分布式[测试执行](/zh-cn/wiki/test-execution/)**对于处理增加的测试负载至关重要。使用支持在多台机器或容器上并行运行测试的工具。这可以缩短反馈周期，确保更快的周转时间。

**协作工具**对于保持开发人员、测试人员和利益相关者之间的沟通至关重要。这些工具应该支持从需求到测试和代码库的追踪，确保所有相关方对验收标准达成一致。

**度量和报告**应该定制以提供有关规模进展和质量的见解。自动仪表板可以跟踪[测试覆盖率](/zh-cn/wiki/test-coverage/)、通过/失败率以及随时间变化的趋势，有助于早期识别潜在问题。
...
- **改进的[测试覆盖率](/zh-cn/wiki/test-coverage/)**：使用覆盖工具确保验收测试覆盖了代码库和用户故事的重要部分。
...
- **回归[测试套件](/zh-cn/wiki/test-suite/)执行时间**：跟踪运行回归套件所需的时间。ATDD 应该导致更有效和有针对性的测试，从而减少执行时间。
