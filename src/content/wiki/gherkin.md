...
- **明智地使用背景**：常见步骤应放在背景部分，但要谨慎使用，以避免隐藏重要信息。
  编写良好的 [gherkin](/zh-cn/wiki/gherkin/) 步骤示例：
...
- **模块化步骤定义**
  ...
  请记住，目标是使[gherkin](/zh-cn/wiki/gherkin/) 场景保持清晰易懂，即使它们代表复杂的[测试用例](/zh-cn/wiki/test-case/)。
...
7. **标记**：[gherkin](/zh-cn/wiki/gherkin/) 中的标记允许选择性 [测试执行](/zh-cn/wiki/test-execution/)，可用于将测试分类为流程中的冒烟、回归或特定功能套件。
  通过将 [gherkin](/zh-cn/wiki/gherkin/) 纳入 CI/CD 流程，团队可确保软件按预期运行，并尽早发现任何偏差，从而在每个版本中保持高质量标准。
