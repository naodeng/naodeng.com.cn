...
[软件测试](/zh-cn/wiki/software-testing/) 中的 **[测试用例](/zh-cn/wiki/test-case/)** 是一组条件或变量，测试人员将在这些条件或变量下确定应用程序、软件系统或其功能之一是否按照最初为其建立的那样工作。 [测试用例](/zh-cn/wiki/test-case/) 的准备是测试过程中的关键步骤，因为它们有助于确保软件按预期运行并满足所有要求。
  [测试用例](/zh-cn/wiki/test-case/) 是测试周期的基础，因为它们提供了记录的测试实例，可以跟踪该实例以供将来复制并确保[功能需求](/zh-cn/wiki/functional-requirements/) 的覆盖范围。它们被设计为尽可能原子，这意味着它们一次测试一件事，并且通常分组为 [测试套件](/zh-cn/wiki/test-suite/) 以进行更好的组织。
  在执行[测试用例](/zh-cn/wiki/test-case/)时，测试人员按照其中概述的步骤来验证特定的功能或特性。然后将结果与 [预期结果](/zh-cn/wiki/expected-result/) 进行比较，以确定测试是否通过或失败。此过程对于识别缺陷并确保软件满足所需的质量标准至关重要。
  在[测试自动化](/zh-cn/wiki/test-automation/)、[测试用例](/zh-cn/wiki/test-case/) 中，使用编程语言或测试工具编写脚本并自动执行，从而允许对应用程序的行为进行频繁且一致的验证。自动化[测试用例](/zh-cn/wiki/test-case/)对于[回归测试](/zh-cn/wiki/regression-testing/)特别有用，其中先前开发和测试的软件会被再次测试，以确保现有功能能够在新的更改下正常工作。

#### 为什么创建测试用例很重要？

除了对 [软件质量](/zh-cn/wiki/software-quality/) 的直接贡献之外，创建 **[测试用例](/zh-cn/wiki/test-case/)** 至关重要。它作为测试的**蓝图**，确保所有功能都得到系统验证。 [测试用例](/zh-cn/wiki/test-case/) 为测试提供**记录的基础**，促进可重复性和可重用性，这在 [回归测试](/zh-cn/wiki/regression-testing/) 以及需要由不同团队成员或在开发生命周期的不同阶段执行测试时尤其重要。
  它们还支持**可追溯性**，将要求与其 [verification](/zh-cn/wiki/verification/) 步骤联系起来，这对于保持符合行业标准和法规至关重要。这种可追溯性确保每个需求都有相应的测试，并且需求的任何更改都可以反映在[测试用例](/zh-cn/wiki/test-case/)中。
  此外，明确定义的[测试用例](/zh-cn/wiki/test-case/)可以成为利益相关者（包括开发人员、测试人员和业务分析师）之间**沟通**​​的一种方式，以确保对正在测试的内容和原因有共同的理解。它们有助于识别测试差距并防止重复测试工作。
  在[测试自动化](/zh-cn/wiki/test-automation/) 的上下文中，[测试用例](/zh-cn/wiki/test-case/) 是创建**自动化[测试脚本](/zh-cn/wiki/test-script/)** 的基础。它们指导 [测试脚本](/zh-cn/wiki/test-script/) 的开发以及适当的自动化工具和框架的选择。
  最后，[测试用例](/zh-cn/wiki/test-case/) 为**估计**测试所需的时间和精力提供了基础，这对于项目规划和管理至关重要。它们还可以作为 [测试覆盖率](/zh-cn/wiki/test-coverage/) 和执行的证据，这对于审计、审查和流程改进很有价值。

#### 测试用例的关键组成部分是什么？

**[测试用例](/zh-cn/wiki/test-case/)** 的关键组件包括：

- **[测试用例](/zh-cn/wiki/test-case/) ID**：用于跟踪的唯一标识符。
- **标题/描述**：测试的简短摘要。
- **先决条件**：执行之前必须满足的任何要求。
- **测试步骤**：详细的执行说明。
- **[测试数据](/zh-cn/wiki/test-data/)** ：测试期间需要的特定输入。
- **[预期结果](/zh-cn/wiki/expected-result/)** ：应用程序行为正确时的预期结果。
- **[实际结果](/zh-cn/wiki/actual-result/)** ：执行期间的实际结果；测试后填写。
- **[后置条件](/zh-cn/wiki/postcondition/)** ：测试执行后应用程序的状态。
...
[测试用例](/zh-cn/wiki/test-case/) 对于确保软件产品的**质量**至关重要。它们充当测试的**蓝图**，详细说明应执行测试的条件和预期结果。通过这些案例仔细验证软件功能的各个方面，测试人员可以识别实际与[预期结果](/zh-cn/wiki/expected-result/)之间的差异，这表明存在缺陷或[bugs](/zh-cn/wiki/bug/)。
  [测试用例](/zh-cn/wiki/test-case/) 的聚合形成了**全面的[测试套件](/zh-cn/wiki/test-suite/)**，涵盖了软件的各个方面，包括功能性、非功能性、积极和消极场景。这种广泛的覆盖范围确保了软件在不同的条件下进行检查，这有助于发现在常规使用过程中可能不明显的隐藏问题。
  此外，[测试用例](/zh-cn/wiki/test-case/) 有助于 **[回归测试](/zh-cn/wiki/regression-testing/)** 进程。随着软件的发展，[测试用例](/zh-cn/wiki/test-case/) 可以重新执行，以确认新的更改不会对现有功能产生不利影响。这对于长期维护[软件质量](/zh-cn/wiki/software-quality/) 至关重要。
  结构良好的[测试用例](/zh-cn/wiki/test-case/) 提供的**可追溯性**也提高了产品的质量。它们可以链接回特定要求，确保满足所有客户期望并且不会忽视任何关键功能。
  本质上，[测试用例](/zh-cn/wiki/test-case/) 是[软件质量](/zh-cn/wiki/software-quality/) 验证的基础。它们提供了结构化的测试方法，促进全面覆盖，支持[回归测试](/zh-cn/wiki/regression-testing/)，并确保需求的可追溯性，所有这些对于交付高质量的软件产品都是至关重要的。

#### 测试用例和测试场景有什么区别？

**[测试用例](/zh-cn/wiki/test-case/)** 和 **[测试场景](/zh-cn/wiki/test-scenario/)** 之间的区别在于它们的范围和细节。 **[测试用例](/zh-cn/wiki/test-case/)** 是一组特定的操作、条件和输入，用于验证软件的特定特性或功能。它很详细，包括 [预期结果](/zh-cn/wiki/expected-result/) 以确定某个功能是否正常工作。
  相反，**[测试场景](/zh-cn/wiki/test-scenario/)** 是要测试的功能的高级描述。它概述了用户可能遇到的情况，但没有深入探讨要采取的具体步骤。 [测试场景](/zh-cn/wiki/test-scenario/) 范围更广，用于确保测试时考虑所有可能的情况。
  [测试用例](/zh-cn/wiki/test-case/) 是 **“如何”** 指南，而 [测试场景](/zh-cn/wiki/test-scenario/) 则更多是 **“做什么”** 指南。场景涵盖多个[测试用例](/zh-cn/wiki/test-case/)，因为它们概述了可以通过多种不同方式进行测试的情况。 [测试场景](/zh-cn/wiki/test-scenario/) 帮助识别[测试用例](/zh-cn/wiki/test-case/)，这反过来又用于应用程序的详细测试。
  例如，[测试场景](/zh-cn/wiki/test-scenario/) 可能是“验证登录功能”。在这种情况下，可以创建多个[测试用例](/zh-cn/wiki/test-case/)，例如：
...
创建 [测试用例](/zh-cn/wiki/test-case/) 涉及以下步骤：
...
在[测试执行](/zh-cn/wiki/test-execution/) 中，**[测试用例](/zh-cn/wiki/test-case/)** 充当测试过程的基本单元。它充当一组特定的条件，在这些条件下测试人员将确定应用程序的特定方面是否按预期工作。每个[测试用例](/zh-cn/wiki/test-case/) 均单独且隔离地执行，以确保它验证其要测试的功能。
  在执行期间，[测试用例](/zh-cn/wiki/test-case/) 为测试人员提供了清晰的步骤顺序，其中包括前提条件、输入值和[预期结果](/zh-cn/wiki/expected-result/)。这种结构化方法确保测试是可重复的，并且可以在不同的测试周期或由不同的测试人员一致地运行。
  [测试用例](/zh-cn/wiki/test-case/) 的执行会导致以下结果之一：
...
要创建和管理[测试用例](/zh-cn/wiki/test-case/)，可以使用各种工具来满足不同的需求和偏好。以下是一些流行工具的列表：
...
软件[测试自动化](/zh-cn/wiki/test-automation/) 中不同类型的[测试用例](/zh-cn/wiki/test-case/) 包括：

- **单元[测试用例](/zh-cn/wiki/test-case/)**：以代码的各个组件或函数为目标，以验证每个部分是否独立正确运行。
- **集成[测试用例](/zh-cn/wiki/test-case/)**：确保多个组件或系统按预期协同工作。
- **系统[测试用例](/zh-cn/wiki/test-case/)**：验证完整且集成的软件产品，以检查其是否满足指定要求。
- **Smoke [测试用例](/zh-cn/wiki/test-case/)**：也称为“[构建验证测试](/zh-cn/wiki/build-verification-testing/)”，它们是 [测试用例](/zh-cn/wiki/test-case/) 的子集，用于验证应用程序的基本功能，以确保其稳定以进行进一步测试。
- **回归[测试用例](/zh-cn/wiki/test-case/)**：旨在检查新代码更改是否对现有功能产生不利影响。
- **性能@@PR​​OTECTED_18@@**：评估应用程序在特定工作负载下的响应能力、稳定性、可扩展性和速度。
- **加载[测试用例](/zh-cn/wiki/test-case/)**：通过模拟多个用户同时访问应用程序来确定系统在正常和峰值条件下的行为方式。
- **强调[测试用例](/zh-cn/wiki/test-case/)**：使系统超出其正常运行能力，通常达到临界点，以确定其阈值。
- **安全[测试用例](/zh-cn/wiki/test-case/)**：识别软件中可能导致安全漏洞的漏洞。
- **可用性[测试用例](/zh-cn/wiki/test-case/)**：评估应用程序的用户界面和用户体验，以确保其用户友好。
...
- **单元[测试用例](/zh-cn/wiki/test-case/)**：以代码的各个组件或函数为目标，以验证每个部分是否独立正确运行。
- **集成[测试用例](/zh-cn/wiki/test-case/)**：确保多个组件或系统按预期协同工作。
- **系统[测试用例](/zh-cn/wiki/test-case/)**：验证完整且集成的软件产品，以检查其是否满足指定的要求。
- **Smoke [测试用例](/zh-cn/wiki/test-case/)**：也称为“[构建验证测试](/zh-cn/wiki/build-verification-testing/)”，它们是 [测试用例](/zh-cn/wiki/test-case/) 的子集，用于验证应用程序的基本功能，以确保其稳定以进行进一步测试。
- **回归[测试用例](/zh-cn/wiki/test-case/)**：旨在检查新代码更改是否对现有功能产生不利影响。
- **性能@@PR​​OTECTED_38@@**：评估应用程序在特定工作负载下的响应能力、稳定性、可扩展性和速度。
- **加载[测试用例](/zh-cn/wiki/test-case/)**：通过模拟多个用户同时访问应用程序来确定系统在正常和峰值条件下的行为方式。
- **压力[测试用例](/zh-cn/wiki/test-case/)**：使系统超出其正常运行能力，通常达到临界点，以确定其阈值。
- **安全[测试用例](/zh-cn/wiki/test-case/)**：识别软件中可能导致安全漏洞的漏洞。
- **可用性[测试用例](/zh-cn/wiki/test-case/)**：评估应用程序的用户界面和用户体验，以确保其用户友好。
- **兼容性[测试用例](/zh-cn/wiki/test-case/)**：检查软件是否在不同设备、浏览器、操作系统等上按预期运行。
- **探索性[测试用例](/zh-cn/wiki/test-case/)**：基于测试人员的知识、经验、分析技能和直觉，在没有预定义步骤的情况下探索软件的功能。
- **[API](/zh-cn/wiki/api/) [测试用例](/zh-cn/wiki/test-case/)**：通过测试 [API](/zh-cn/wiki/api/) 及其交互来验证应用程序内构建架构的逻辑。
- **UI [测试用例](/zh-cn/wiki/test-case/)**：专注于图形界面以及用户如何与其交互，确保元素可见、操作可行且 UI 正确响应。
...
**功能@@PR​​OTECTED_1@@** 是为验证软件应用程序的特定特性或功能而执行的一组操作。与关注性能、安全性或可用性的非功能性[测试用例](/zh-cn/wiki/test-case/) 不同，功能性[测试用例](/zh-cn/wiki/test-case/) 关注系统的功能。他们通过向软件提供输入并检查输出来针对[功能需求](/zh-cn/wiki/functional-requirements/) 验证软件。
  功能@@PR​​OTECTED_5@@ 通常以粒度级别编写，以覆盖应用程序的各个功能或部分。它们可以是积极的，测试系统对预期输入的响应，也可以是消极的，确保系统妥善处理错误或意外的输入。
  要编写函数[测试用例](/zh-cn/wiki/test-case/)，您需要：
...
**非功能性[测试用例](/zh-cn/wiki/test-case/)** 侧重于定义系统**如何**运行的软件系统的各个方面，而不是系统**做什么**。这些[测试用例](/zh-cn/wiki/test-case/) 与系统在某些约束下的行为有关，包括性能、安全性、可用性、可靠性和可扩展性等属性。
  与验证特定操作或功能的功能性[测试用例](/zh-cn/wiki/test-case/) 不同，非功能性[测试用例](/zh-cn/wiki/test-case/) 验证系统的非[功能需求](/zh-cn/wiki/functional-requirements/)，这些非[功能需求](/zh-cn/wiki/functional-requirements/) 与任何特定功能或用户操作无关。他们确保软件满足一定的质量和用户体验标准。
...
编写有效[测试用例](/zh-cn/wiki/test-case/) 的最佳实践包括：

- **清晰简洁**：写出简单易懂的[测试用例](/zh-cn/wiki/test-case/)。避免歧义，以确保任何人都可以执行它们。
- **使用描述性名称**：选择反映[测试用例](/zh-cn/wiki/test-case/)用途的名称，以便更容易一目了然地识别其意图。
- **优先考虑[测试用例](/zh-cn/wiki/test-case/)**：根据业务影响、关键功能和故障可能性订购[测试用例](/zh-cn/wiki/test-case/)。
- **包括先决条件**：在执行测试之前清楚地说明任何所需的状态或所需的配置。
- **定义[预期结果](/zh-cn/wiki/expected-result/)**：指定正确的结果应该是什么，以便毫无疑问测试是否通过或失败。
- **使它们独立**：每个[测试用例](/zh-cn/wiki/test-case/) 应该是独立的，而不是依赖于另一个的结果。
- **参数化数据**：使用数据驱动的测试来避免硬编码值，从而提高灵活性并减少维护。
- **版本控制**：将[测试用例](/zh-cn/wiki/test-case/)置于版本控制之下以跟踪更改并维护历史记录。
- **同行评审**：由同行评审 [测试用例](/zh-cn/wiki/test-case/) 以发现错误并提高质量。
- **适当时自动化**：自动化重复、需要精确或需要频繁运行的[测试用例](/zh-cn/wiki/test-case/)。
- **维护可追溯性**：将 [测试用例](/zh-cn/wiki/test-case/) 链接到需求或用户故事，以确保覆盖范围并促进 [影响分析](/zh-cn/wiki/impact-analysis/)。
...
- **优先考虑[测试用例](/zh-cn/wiki/test-case/)**：根据业务影响、关键功能和故障可能性订购[测试用例](/zh-cn/wiki/test-case/)。
- **包括先决条件**：在执行测试之前清楚地说明任何所需的状态或所需的配置。
- **定义[预期结果](/zh-cn/wiki/expected-result/)**：指定正确的结果应该是什么，以便毫无疑问测试是通过还是失败。
- **使它们独立**：每个[测试用例](/zh-cn/wiki/test-case/) 应该是独立的，而不是依赖于另一个的结果。
- **参数化数据**：使用数据驱动的测试来避免硬编码值，从而提高灵活性并减少维护。
- **版本控制**：将[测试用例](/zh-cn/wiki/test-case/)置于版本控制之下以跟踪更改并维护历史记录。
- **同行评审**：由同行评审 [测试用例](/zh-cn/wiki/test-case/) 以发现错误并提高质量。
...
确保[测试用例](/zh-cn/wiki/test-case/)涵盖所有可能的场景涉及多种技术的组合：

- **[等价划分](/zh-cn/wiki/equivalence-partitioning/)** ：将输入分为应同等对待的组。测试每个分区的一名代表。
- **边界值分析**：关注输入范围的边缘情况，因为边界处经常出现错误。
- **[决策表测试](/zh-cn/wiki/decision-table-testing/)** ：创建一个涵盖输入和相应操作的所有可能组合的表。
- **[状态转换测试](/zh-cn/wiki/state-transition-testing/)** ：识别所有可能的状态和转换，以确保所有路径都经过测试。
- **[用例测试](/zh-cn/wiki/use-case-testing/)** ：基于真实使用场景进行测试以涵盖功能要求。
- **组合测试**：使用成对测试等工具生成涵盖所有输入参数组合的最小测试用例集。
- **[基于风险的测试](/zh-cn/wiki/risk-based-testing/)** ：根据故障的可能性和影响确定测试的优先级。
...
- **[决策表测试](/zh-cn/wiki/decision-table-testing/)** ：创建一个涵盖输入和相应操作的所有可能组合的表。
- **[状态转换测试](/zh-cn/wiki/state-transition-testing/)** ：识别所有可能的状态和转换，以确保所有路径都经过测试。
- **[用例测试](/zh-cn/wiki/use-case-testing/)** ：基于真实使用场景进行测试以涵盖功能需求。
- **组合测试**：使用成对测试等工具生成涵盖所有输入参数组合的最小测试用例集。
- **[基于风险的测试](/zh-cn/wiki/risk-based-testing/)** ：根据故障的可能性和影响确定测试的优先级。
- **[探索性测试](/zh-cn/wiki/exploratory-testing/)** ：通过临时会话补充结构化测试，以发现正式方法可能遗漏的场景。
- **用户故事和验收标准**：确保测试用例符合用户期望和业务需求。
...
创建[测试用例](/zh-cn/wiki/test-case/)时要避免的常见错误：

- **忽略[测试用例](/zh-cn/wiki/test-case/) 独立性**：每个测试用例应该是独立的并且独立于其他测试用例，以避免级联故障。
- **歧义**：测试用例必须清晰且精确。不明确的步骤可能会导致执行和结果不一致。
- **过多的细节**：虽​​然清晰度很重要，但过多的细节会使测试用例难以维护。仅包括理解和执行所必需的内容。
- **忽略[负面测试](/zh-cn/wiki/negative-testing/)** ：仅关注积极的情况可能会错过潜在的缺陷。包括负面测试用例以确保稳健的测试。
...
- **忽略[测试环境](/zh-cn/wiki/test-environment/)** ：不指定所需的测试环境可能会因环境差异而导致误报或误报。
- **无法更新[测试用例](/zh-cn/wiki/test-case/)**：随着软件的发展，测试用例也应该发展。需要定期更新以保持相关性。
- **不考虑维护**：测试用例应该易于维护。避免复杂的结构，这会使维护成为一场噩梦。
- **忽略[测试用例](/zh-cn/wiki/test-case/) 独立性**：每个测试用例应该是独立的并且独立于其他测试用例，以避免级联失败。
- **歧义**：测试用例必须清晰且精确。不明确的步骤可能会导致执行和结果不一致。
- **过多的细节**：虽​​然清晰度很重要，但过多的细节会使测试用例难以维护。仅包括理解和执行所必需的内容。
- **忽略[负面测试](/zh-cn/wiki/negative-testing/)** ：仅关注积极的情况可能会错过潜在的缺陷。包括负面测试用例以确保稳健的测试。
- **不优先考虑**：所有测试用例并不相同。根据风险、功能重要性和使用频率对它们进行优先级排序。
...
- **不规划可重用性**：设计测试用例时考虑可重用性，从长远来看可以节省时间和精力。
- **忽视数据管理**：硬编码测试数据可能会限制测试的适用性。尽可能使用数据驱动的方法。
- **忽略[测试环境](/zh-cn/wiki/test-environment/)** ：未指定所需的测试环境可能会因环境差异而导致误报或误报。
- **无法更新[测试用例](/zh-cn/wiki/test-case/)**：随着软件的发展，测试用例也应该发展。需要定期更新以保持相关性。
- **不考虑维护**：测试用例应该易于维护。避免复杂的结构，这会使维护成为一场噩梦。

#### 测试用例应该多久审查和更新一次？

[测试用例](/zh-cn/wiki/test-case/) 应**定期**进行审查和更新，以确保它们保持有效和相关。评论的频率可能受到以下几个因素的影响：
...
为了提高[测试自动化](/zh-cn/wiki/test-automation/)中[测试用例](/zh-cn/wiki/test-case/)的**可重用性**：
...
- **实现 [页面对象模型](/zh-cn/wiki/page-object-model/) (POM)** ：将 UI 结构和行为封装在页面对象中。这减少了重复工作，并且在 UI 更改时使维护变得更加容易。
- **参数化测试**：使用参数来概括测试用例，使其适用于不同的情况。
- **采用版本控制最佳实践**：在版本控制系统中组织[测试脚本](/zh-cn/wiki/test-script/)，具有清晰的命名约定和目录结构，以方便共享和重用[测试用例](/zh-cn/wiki/test-case/)。
- **记录可重用组件**：确保所有可重用模块、函数和页面对象都有详细记录，使其他人更容易理解和使用它们。
