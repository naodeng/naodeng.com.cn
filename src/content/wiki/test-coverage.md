...
description: "分支覆盖率（Branch coverage），也称为 判定覆盖率 (decision coverage)，确保每个判定点的每个可能分支至少被执行一次。例如，在 `if` 语句的上下文中，应当测试 true 和 false 分支。要应用分支覆盖率，需识别代码中所有的判定点…"
section: "T"
related:
  - code-coverage
  - manual-testing
...
<!-- TOC END -->
测试覆盖率测量测试的程序代码部分。它标识了期间执行的代码部分测试用例，确保全面评估。
...
- **Coveralls** ：与 Codecov 类似，它与 GitHub 集成以跟踪一段时间内的代码覆盖率。 这些工具可以集成到 CI/CD 管道中以自动生成覆盖率报告。他们通常通过仪表板、详细的文件细分和历史数据跟踪来提供见解。选择正确的工具取决于编程语言、现有的开发环境以及与其他开发工具所需的集成级别。
...
- **监控和分析测试结果**确定趋势和需要改进的领域，从而有针对性地增加覆盖范围。
...
- **重构测试**更新代码以保持其干净、可读和可维护。这使得随着应用程序的增长更容易扩展覆盖范围。

- **优先测试**基于关键路径和风险领域。重点关注对应用程序性能和用户体验影响最大的功能。

- **尽可能自动化** ，但要有选择性。自动执行重复且耗时的测试，但避免对需要人工判断的复杂场景进行自动化。
...
- **鼓励测试文化**开发人员为他们的代码编写单元测试，有助于整体覆盖。
...
- **设定覆盖目标**并跟踪进度，但避免以 100% 覆盖为目标，因为这可能不具有成本效益。相反，目标是提供有意义的覆盖范围，以提供对应用程序稳定性的信心。
