...
- **指标**：提供有价值的指标，例如代码覆盖率，可以指导开发和维护工作。
  通过利用[单元测试框架](/zh-cn/wiki/unit-test-framework/)，[测试自动化](/zh-cn/wiki/test-automation/) 工程师可以确保软件经过彻底、高效的测试，从而产生更高质量和更易于维护的代码。它是现代软件开发生命周期中不可或缺的工具。
...
- **注释**：提供一种向测试方法添加元数据的方法，例如对测试进行分类或将方法标记为[测试用例](/zh-cn/wiki/test-case/)。
  使用假设框架的简单 [测试用例](/zh-cn/wiki/test-case/) 示例：
...
9. 如果需要，使用框架的装饰器或属性**注释**测试，以指定测试元数据，例如类别或预期异常。
  以下是使用 [Jest](/zh-cn/wiki/jest/) 的 TypeScript 示例：
...
9. **重复该过程**
  ...
  以下是在调试模式下启动 [node.js](/zh-cn/wiki/node-js/) 测试的命令示例：
...
- **代码质量平台**：与 SonarQube 等平台连接以分析测试结果和测试后执行后的代码质量。
  通过将 [单元测试框架](/zh-cn/wiki/unit-test-framework/) 与这些工具集成，您可以确保测试成为开发工作流程中不可或缺的一部分，从而尽早发现问题并维护代码质量。
...
8. **查看框架生成的 [测试报告](/zh-cn/wiki/test-report/)** 以分析通过/失败状态和覆盖率指标。
  通过执行这些步骤，您可以有效地自动化[单元测试](/zh-cn/wiki/unit-testing/)，确保您的代码经过彻底测试且可靠。
...
- **及时** ：在接近编写相应代码的时间编写测试。
  ...
  **在必要时重构测试**以提高可读性和[可维护性](/zh-cn/wiki/maintainability/)而不改变行为。
  **使用 [代码覆盖率](/zh-cn/wiki/code-coverage/) 工具**来识别代码库中未经测试的部分，但不要以牺牲测试质量为代价来实现 100% 的覆盖率。
