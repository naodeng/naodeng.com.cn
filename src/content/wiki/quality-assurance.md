...
测试在[质量保证](/zh-cn/wiki/quality-assurance/) (QA) 中起着**关键作用**，因为它涉及软件的**[verification](/zh-cn/wiki/verification/)** 和**验证**，以确保其满足指定的要求和标准。这是一个**主动的过程**，旨在识别与实际产品功能相比的缺陷、差距或缺失的需求。
  在 QA 中，测试用于：
...
- **支持持续改进**
    通过提供质量反馈并建议需要改进的领域。
  QA 中的测试不仅仅是寻找[bugs](/zh-cn/wiki/bug/)，而且还要**提供保证**，确保软件将在现实世界中按预期运行。这种保证对于维持客户信任和满意度至关重要。
  [自动化测试](/zh-cn/wiki/automated-testing/) 是 [测试自动化](/zh-cn/wiki/test-automation/) 工程师工作的关键组成部分，它通过以下方式增强质量保证：
...
- **设计[测试计划](/zh-cn/wiki/test-plan/) 和[测试用例](/zh-cn/wiki/test-case/)**
    涵盖软件的功能、性能和安全方面。
...
- **维护[测试环境](/zh-cn/wiki/test-environment/)**
    尽可能地反映生产情况。
...
- **监控和报告[测试执行](/zh-cn/wiki/test-execution/)**
...
- **[CMMI](/zh-cn/wiki/cmmi/)（能力成熟度模型集成）**：[CMMI](/zh-cn/wiki/cmmi/) 是一个流程级别改进培训和评估计划，可帮助组织改进其软件质量保证流程。
  遵守这些标准有助于 QA 专业人员将其 [测试自动化](/zh-cn/wiki/test-automation/) 策略与行业最佳实践保持一致，确保软件产品满足客户期望和监管要求。
