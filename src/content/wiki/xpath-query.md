...
- **轴说明符** ：定义节点之间的树关系，如
    ...
    `following-sibling::`
    。
  [XPath 查询](/zh-cn/wiki/xpath-query/) 结构示例：
...
- **合并函数**：使用 XPath 函数进行字符串操作、数值计算或节点集处理（例如 `text()`、`contains()`、`count()`）。
  以下是结构化 [XPath 查询](/zh-cn/wiki/xpath-query/) 的示例：
...
- **self** ：选择当前节点。
  [XPath 查询](/zh-cn/wiki/xpath-query/) 中的用法示例：
...
- **数字函数**：执行数字运算，例如，
    ...
    `round()`
    。
  在[测试自动化](/zh-cn/wiki/test-automation/)中，函数用于增强节点的选择。例如，要查找具有特定属性值的元素，您可以使用：
...
3. **执行[XPath 查询](/zh-cn/wiki/xpath-query/)**
...
- **错误处理**：实现安全的错误处理，即使检测到 XPath 注入尝试，也不会暴露敏感信息。
  通过结合这些策略，您可以显着降低软件 [测试自动化](/zh-cn/wiki/test-automation/) 中 XPath 注入漏洞的风险。
