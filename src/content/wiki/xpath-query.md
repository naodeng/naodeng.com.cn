...
[XPath 查询](/zh-cn/wiki/xpath-query/) 与其他查询语言的不同之处主要在于它对 XML 和 HTML 结构的**特异性。与 [SQL](/zh-cn/wiki/sql/)（设计用于查询关系 [databases](/zh-cn/wiki/database/)）或 CSS 选择器（用于在 HTML 中设置样式和选择元素）不同，XPath 允许根据各种条件（包括节点的层次结构、属性和 XML 文档中的内容）**选择节点**。
...
在处理[databases](/zh-cn/wiki/database/) 中支持XML 查询和操作的XML 数据类型时，[XPath 查询](/zh-cn/wiki/xpath-query/) 可以与[SQL](/zh-cn/wiki/sql/) 集成。此集成允许提取和操作存储在 [SQL](/zh-cn/wiki/sql/) [databases](/zh-cn/wiki/database/) 中的 XML 数据。
...
在此[SQL](/zh-cn/wiki/sql/) 查询中，`XPath_Expression` 是您放置[XPath 查询](/zh-cn/wiki/xpath-query/) 来过滤或选择特定XML 数据的位置，`XPath_Node_Selection` 是用于标识将表示为行的节点的XPath 表达式。
  通过在 [SQL](/zh-cn/wiki/sql/) 查询中利用 XPath，您可以执行涉及关系数据结构和分层数据结构的复杂查询，为需要与传统 [SQL](/zh-cn/wiki/sql/) 查询结合查询 XML 数据的场景提供强大的工具。这在 XML 数据存储在 [SQL](/zh-cn/wiki/sql/) [databases](/zh-cn/wiki/database/) 中的报告、数据迁移或集成任务等场景中特别有用。
