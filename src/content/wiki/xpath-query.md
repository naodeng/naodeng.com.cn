...
[XPath 查询](/zh-cn/wiki/xpath-query/) 是一种功能强大的语言，设计用于从 XML 文档中选择节点。它在查询 HTML 结构时同样有效，特别是在 [网络自动化](/zh-cn/wiki/web-automation/) 框架（如 [selenium](/zh-cn/wiki/selenium/)）中使用时。 XPath 因其精确执行复杂查询的能力而脱颖而出，使测试人员能够精确地定位网页 DOM 中的元素。
...
XPath 能够使用各种轴（例如 `ancestor`、`descendant`、`following` 和 `preceding`）导航 DOM，为测试人员提供了一个通用工具包，可以根据 Web 元素在 DOM 树中的位置与它们进行交互。当元素缺乏唯一标识符或 DOM 结构是动态时，这特别有用。
  在[测试自动化](/zh-cn/wiki/test-automation/) 中，XPath 查询通常在脚本中使用来与 Web 元素交互，例如单击按钮或提取文本。 XPath 的精确性使其对​​于断言元素或其状态的存在非常有价值，这对于验证 Web 应用程序的功能至关重要。
  虽然 XPath 是 [网络自动化](/zh-cn/wiki/web-automation/) 的基石，但它需要精心设计，以避免可能因 UI 更改而破坏的脆弱选择器。测试人员必须平衡特异性和灵活性，通常选择能够承受 DOM 结构中微小变化的相对 XPath 表达式。有效使用 XPath 可以显着增强自动化 [测试套件](/zh-cn/wiki/test-suite/) 的稳健性和 [可维护性](/zh-cn/wiki/maintainability/)。

#### 为什么 XPath 查询在软件自动化中很重要？

[XPath 查询](/zh-cn/wiki/xpath-query/) 在软件[测试自动化](/zh-cn/wiki/test-automation/) 中至关重要，因为它在 XML 和 HTML 文档中定位元素时具有**精确性**和**灵活性**。它使测试人员能够识别具有**特定属性**、**文本值**或**层次关系**的元素，这在处理元素属性或位置可能发生变化的动态或复杂网页时至关重要。
  ...
  在 **[selenium](/zh-cn/wiki/selenium/)** 和其他 [网络自动化](/zh-cn/wiki/web-automation/) 工具的上下文中，XPath 通常是**首选**查询语言，因为它具有**跨浏览器兼容性**和对**复杂定位器**的支持。它也是用于**断言**的强大工具，允许测试人员验证页面中元素的存在、不存在或状态。
  然而，与 CSS 选择器相比，XPath 查询可能**脆弱**且**慢**，尤其是对于结构不良的 HTML。为了缓解这种情况，编写**高效**和**弹性** XPath 表达式非常重要，重点关注**相对路径**和**鲁棒属性**。
  总之，XPath 是 [测试自动化](/zh-cn/wiki/test-automation/) 工程师的工具库中不可或缺的工具，它提供了在自动化测试中有效地与 UI 元素交互和验证 UI 元素所需的**粒度**和**控制**。

#### XPath 查询的基本组件是什么？

[XPath 查询](/zh-cn/wiki/xpath-query/) 的基本组件包括：
...
- **轴说明符** ：定义节点之间的树关系，如 `child::` , `ancestor::` , 或 `following-sibling::` 。 [XPath 查询](/zh-cn/wiki/xpath-query/) 结构示例：
...
在端到端 (e2e) 测试中，**[XPath 查询](/zh-cn/wiki/xpath-query/)** 在定位 Web 元素并与之交互方面发挥着至关重要的作用。它使测试人员能够精确定位网页的文档对象模型 (DOM) 中的特定元素，这对于模拟用户交互（例如单击按钮、输入文本以及验证元素的存在或属性）至关重要。
  ...
  在 [selenium](/zh-cn/wiki/selenium/) 等自动化 e2e 测试框架中，XPath 通常用于创建强大的定位器。例如：

```
  driver.findElement(By.xpath("//button[text()='Submit']")).click();
```
这行代码将找到一个带有文本“提交”的按钮并执行单击操作，模仿用户在 [测试场景](/zh-cn/wiki/test-scenario/) 期间的交互。
  总体而言，[XPath 查询](/zh-cn/wiki/xpath-query/) 对于在 e2e 测试中实现元件定位策略的精确性和灵活性是不可或缺的，有助于实现更可靠和可维护的[测试套件](/zh-cn/wiki/test-suite/)。

#### XPath 查询与其他查询语言有何不同？

[XPath 查询](/zh-cn/wiki/xpath-query/) 与其他查询语言的不同之处主要在于它对 XML 和 HTML 结构的**特异性。与 [SQL](/zh-cn/wiki/sql/)（设计用于查询关系 [数据库](/zh-cn/wiki/database/)）或 CSS 选择器（用于在 HTML 中设置样式和选择元素）不同，XPath 允许根据各种条件（包括节点的层次结构、属性和 XML 文档中的内容）**选择节点**。
  ...
  此外，XPath 使用**谓词**的能力提供了比 CSS 选择器中通常提供的更精细的选择控制级别。这使得它对于需要精确提取数据的场景特别强大，例如在需要针对特定​​元素的[测试自动化](/zh-cn/wiki/test-automation/)中。
  与用于 JSON 对象的 JSONPath 不同，XPath 是针对 XML 的结构化特性而设计的，不能直接应用于 JSON。但是，两者都具有相似的路径表达式概念来导航元素。
  总体而言，XPath 的独特功能使其成为需要在 XML 和 HTML 文档中**精确导航和选择**的场景中不可或缺的工具，特别是在软件[测试自动化](/zh-cn/wiki/test-automation/) 的上下文中。

### 语法和结构

#### XPath 查询的基本语法是什么？

[XPath 查询](/zh-cn/wiki/xpath-query/) 的基本语法由路径表达式组成，该表达式定义在 XML 文档中导航元素和属性的方式。这是一个简化的细分：
...
要构造[XPath 查询](/zh-cn/wiki/xpath-query/)来选择节点，请遵循以下准则：
...
- **合并函数**：使用 XPath 函数进行字符串操作、数值计算或节点集处理（例如 `text()`、`contains()`、`count()`）。 以下是结构化 [XPath 查询](/zh-cn/wiki/xpath-query/) 的示例：
...
- **self** ：选择当前节点。 [XPath 查询](/zh-cn/wiki/xpath-query/) 中的用法示例：

```
  //book/child::*
```
这将选择 `book` 节点的所有子元素。 [测试自动化](/zh-cn/wiki/test-automation/) 工程师使用这些轴在 XML 或 HTML 文档结构中导航，从而可以精确定位用于交互和验证的元素。
...
```
  //book/author[1]/name
```
有效使用谓词可以实现更精确、更高效的 XPath 查询，这对于 [测试自动化](/zh-cn/wiki/test-automation/) 中准确定位元素并对元素执行操作或验证至关重要。
...
- **数字函数**：执行数字运算，例如， `sum()` , `floor()` , `round()` 。 在[测试自动化](/zh-cn/wiki/test-automation/)中，函数用于增强节点的选择。例如，要查找具有特定属性值的元素，您可以使用：
...
`not()` 函数反转`@checked` 谓词的布尔结果。
  正确使用函数可以大大提高 [测试自动化](/zh-cn/wiki/test-automation/) 脚本中 XPath 查询的精度和效率。
...
在 [selenium](/zh-cn/wiki/selenium/) 中使用 **XPath** 进行 [网络自动化](/zh-cn/wiki/web-automation/) 测试涉及通过元素的 XML 路径来定位网页上的元素。以下是有关如何实现此目的的简明指南：
...
```
通过执行这些步骤，您可以有效地利用 [selenium](/zh-cn/wiki/selenium/) 中的 XPath 来定位和操作 Web 元素，以满足您的自动化测试需求。
...
[XPath 查询](/zh-cn/wiki/xpath-query/) 通常用于软件[测试自动化](/zh-cn/wiki/test-automation/)，用于以下[用例](/zh-cn/wiki/use-case/)：
...
```
这些[用例](/zh-cn/wiki/use-case/) 展示了 XPath 在解决软件[测试自动化](/zh-cn/wiki/test-automation/) 中遇到的各种场景方面的多功能性。
...
[XPath 查询](/zh-cn/wiki/xpath-query/) 可用于通过提供一种选择和遍历文档树中的节点的方法来导航 XML 文档。它允许精确定位 XML 结构中的特定元素、属性或文本，从而实现精确的数据提取或操作。
...
```
使用[测试自动化](/zh-cn/wiki/test-automation/) 中的 XPath，工程师可以断言 XML 响应或配置的存在、值和结构，确保软件在与基于 XML 的数据交互时按预期运行。

#### 使用 XPath 查询有哪些挑战以及如何克服这些挑战？

XPath 查询可能会给 [测试自动化](/zh-cn/wiki/test-automation/) 带来一些挑战：

- **性能问题**：复杂的 XPath 表达式可能会很慢，尤其是对于大型文档。为了克服这个问题，请使用更具体的路径并避免使用通配符。通过针对具有独特属性的元素进行优化。
- **[可维护性](/zh-cn/wiki/maintainability/)**：XPath 可能很脆弱，会随着 UI 更改而中断。尽可能使用`id` 或`name` 等稳定标识符。在适当的情况下使用 CSS 选择器可以增强 [可维护性](/zh-cn/wiki/maintainability/)。
...
- **跨浏览器兼容性**：不同的浏览器对 XPath 的解释可能略有不同。通过跨浏览器测试 XPath 并使用 [selenium](/zh-cn/wiki/selenium/) 等工具来消除一些差异，确保跨浏览器兼容性。
...
```
通过采用战略方法应对这些挑战，XPath 可以成为定位 [测试自动化](/zh-cn/wiki/test-automation/) 中元素的强大工具。

#### 如何使用 XPath Query 从 HTML 中提取数据？

[XPath 查询](/zh-cn/wiki/xpath-query/) 可用于通过定位 HTML DOM 结构中的特定元素、属性或文本来**从 HTML 中提取数据**。由于 HTML 在结构上与 XML 类似，因此可以有效地应用 XPath 来遍历 HTML 树并选择感兴趣的节点。
  提取数据：
...
3. **执行[XPath 查询](/zh-cn/wiki/xpath-query/)**使用支持 XPath 的工具或库，例如 Python 中的 Selenium 或 lxml。 例如，要从具有特定 `id` 的段落中提取文本，您可以使用：
...
```
在 [测试自动化](/zh-cn/wiki/test-automation/) 框架（如 [selenium](/zh-cn/wiki/selenium/)）中，您可以将这些 XPath 表达式与 `find_element_by_xpath()` 等方法一起使用来与 Web 元素进行交互：
...
3. **执行[XPath 查询](/zh-cn/wiki/xpath-query/)**使用支持 XPath 的工具或库，例如 Python 中的 Selenium 或 lxml。
...
```
    //input[not(@disabled)]
```
这些函数增强了直接在 XPath 查询中执行文本操作、字符串比较和数学计算的能力，使其成为[测试自动化](/zh-cn/wiki/test-automation/) 工程师的强大工具。
...
1. **注册命名空间**：在执行[XPath 查询](/zh-cn/wiki/xpath-query/)之前，使用XML解析库提供的[API](/zh-cn/wiki/api/)以前缀注册命名空间。例如，在 Java 的 XPath [API](/zh-cn/wiki/api/) 中，您可以使用 `NamespaceContext` 将前缀映射到名称空间 URI。
2. **在 [XPath 查询](/zh-cn/wiki/xpath-query/) 中使用前缀**：注册后，请在 XPath 表达式中使用前缀来引用命名空间中的元素。
...
```
请记住，前缀的选择是任意的，不必与 XML 文档中使用的前缀相匹配；它只需要在您的 [XPath 查询](/zh-cn/wiki/xpath-query/) 和命名空间注册中保持一致。
...
- **参数化查询**：使用支持参数化的[API](/zh-cn/wiki/api/)，它将查询结构与输入值分开。
...
- **错误处理**：实现安全的错误处理，即使检测到 XPath 注入尝试，也不会暴露敏感信息。 通过结合这些策略，您可以显着降低软件 [测试自动化](/zh-cn/wiki/test-automation/) 中 XPath 注入漏洞的风险。
...
在处理[数据库](/zh-cn/wiki/database/) 中支持XML 查询和操作的XML 数据类型时，[XPath 查询](/zh-cn/wiki/xpath-query/) 可以与[SQL](/zh-cn/wiki/sql/) 集成。此集成允许提取和操作存储在 [SQL](/zh-cn/wiki/sql/) [数据库](/zh-cn/wiki/database/) 中的 XML 数据。
  例如，在 Microsoft [SQL](/zh-cn/wiki/sql/) Server 中，您可以使用 `nodes()` 方法将 XML 数据分解为关系行和列，然后将 XPath 表达式应用于目标特定元素或属性。以下是如何将 XPath 与 [SQL](/zh-cn/wiki/sql/) 结合起来的示例：
...
```
在此[SQL](/zh-cn/wiki/sql/) 查询中，`XPath_Expression` 是您放置[XPath 查询](/zh-cn/wiki/xpath-query/) 来过滤或选择特定XML 数据的位置，`XPath_Node_Selection` 是用于标识将表示为行的节点的XPath 表达式。
  通过在 [SQL](/zh-cn/wiki/sql/) 查询中利用 XPath，您可以执行涉及关系数据结构和分层数据结构的复杂查询，为需要与传统 [SQL](/zh-cn/wiki/sql/) 查询结合查询 XML 数据的场景提供强大的工具。这在 XML 数据存储在 [SQL](/zh-cn/wiki/sql/) [数据库](/zh-cn/wiki/database/) 中的报告、数据迁移或集成任务等场景中特别有用。
