...
**[测试环境](/zh-cn/wiki/test-environment/)** 是受控[环境搭建](/zh-cn/wiki/setup/)，其中执行[软件测试](/zh-cn/wiki/software-testing/)。它包括硬件、软件、网络配置以及模拟类似生产环境的其他必要工具和服务。这个[环境搭建](/zh-cn/wiki/setup/)允许测试人员验证新版本，确保应用程序在各种条件下按预期运行，而不影响实际的生产系统。
  ...
  确保[测试环境](/zh-cn/wiki/test-environment/) 的保真度涉及使其配置与生产保持一致，其中包括软件版本、网络设置和[数据库](/zh-cn/wiki/database/) 副本。基础设施即代码 (IaC) 等工具可以自动化[环境搭建](/zh-cn/wiki/setup/)，从而更容易复制和维护一致性。
...
- **[数据库](/zh-cn/wiki/database/)** ：配置生产数据库的副本或子集，为测试中的应用程序提供真实的数据层。
...
**[测试环境](/zh-cn/wiki/test-environment/)** 是一个单独的[环境搭建](/zh-cn/wiki/setup/)，其中安装软件和应用程序以模拟**生产环境**以进行测试。主要区别在于它们的**目的和用途**。
  **生产环境**是最终用户与最终​​产品交互的实时环境。它针对**安全性、稳定性和性能**进行了优化，以确保无缝的用户体验。这里的任何更改都是严格测试的结果，并且是永久性的。
  ...
  虽然我们努力确保[测试环境](/zh-cn/wiki/test-environment/) 紧密复制生产环境，但由于测试活动的性质，存在固有的差异。这些可以包括**不同的配置、额外的诊断工具和不太严格的安全控制**以方便测试。此外，[测试环境](/zh-cn/wiki/test-environment/) 经常**重置或更新**，以反映生产[环境搭建](/zh-cn/wiki/setup/) 中的变化或为新测试做准备，而生产环境则尽可能保持稳定。
...
**[测试环境](/zh-cn/wiki/test-environment/)** 通过提供受控和隔离的设置，在**软件开发生命周期 (SDLC)** 中发挥关键作用，在该设置中**部署、执行和监视**软件应用程序以验证其在测试条件下的行为。它支持在应用程序发布到生产环境之前对软件功能、性能和稳定性进行**验证**。
  ...
  通过模拟生产环境，[测试环境](/zh-cn/wiki/test-environment/) 有助于识别在开发阶段可能无法检测到的特定于环境的问题。这包括测试应用程序与外部系统、[数据库](/zh-cn/wiki/database/) 以及它将在现实世界中交互的其他服务的交互。
...
设置[测试环境](/zh-cn/wiki/test-environment/) 涉及一系列步骤，以确保它有效支持[测试自动化](/zh-cn/wiki/test-automation/)。首先**定义环境的范围**，包括所需的系统、网络和配置。接下来，**配置必要的硬件**或**分配云资源**，具体取决于您的基础架构选择。
  安装正确版本的**操作系统**和**所需的软件**。这包括 [数据库](/zh-cn/wiki/database/)、Web 服务器和任何其他依赖项。使用 Terraform 或 Ansible 等**基础设施即代码 (IaC)** 工具来实现一致且可重复的[setups](/zh-cn/wiki/setup/)。
  ...
  实施**监控和日志记录**来跟踪环境的运行状况和调试问题。最后，彻底记录环境[环境搭建](/zh-cn/wiki/setup/)** 以提高透明度并帮助故障排除。
...
- **环境奇偶性**：定期同步数据模型、模式和参考数据。使用 [数据库](/zh-cn/wiki/database/) 版本控制工具（例如 Liquibase 或 Flyway）来维护 [数据库](/zh-cn/wiki/database/) 一致性。
...
- **[数据库](/zh-cn/wiki/database/) 架构更改**：只要有修改，就应用更新，以确保数据完整性和兼容性。
- **配置更改**：如果配置文件或环境变量发生更改，请更新环境。
- **安全更新**：应用补丁和安全修复程序以维护环境的安全态势。
- **第三方服务更新**：当集成服务或 API 发布更新时，测试环境应反映这些更改。
...
[测试环境](/zh-cn/wiki/test-environment/) 管理 (TEM) 涉及监督[环境搭建](/zh-cn/wiki/setup/)、[测试环境](/zh-cn/wiki/test-environment/) 的维护和运营，以确保它们为[软件测试](/zh-cn/wiki/software-testing/) 提供稳定且一致的平台。它包括分配资源、管理配置、调度环境使用以及处理环境相关问题。
  **关键活动** TEM 包括：
...
- **[环境搭建](/zh-cn/wiki/setup/) 和维护**：虚拟环境的设置和维护可能更加复杂，而云服务通常提供更简单的部署和管理。
