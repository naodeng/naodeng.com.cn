...
description: "测试脚本 (Test Script) 是由自动化测试工具执行的一组指令，用于验证软件应用程序的功能。它本质上是一个与被测软件交互、模拟用户操作并检查预期结果的程序。"
section: "T"
order: 0
---
...
[测试脚本](/zh-cn/wiki/test-script/) 在[软件测试](/zh-cn/wiki/software-testing/) 中至关重要，因为它们充当自动化测试过程的**可执行指令**。通过自动化重复且耗时的任务，[测试脚本](/zh-cn/wiki/test-script/) 增强了**测试效率**和**一致性**，确保每次都以相同的方式执行测试。它们使测试人员能够在更短的时间内覆盖更多领域，从而增加 **[测试覆盖率](/zh-cn/wiki/test-coverage/)** 和发现缺陷的可能性。
  此外，[测试脚本](/zh-cn/wiki/test-script/) 对于**持续集成**和**持续交付**管道至关重要，允许快速反馈，这在[敏捷开发](/zh-cn/wiki/agile-development/) 环境中至关重要。它们通过快速验证新代码更改是否不会对现有功能产生不利影响来促进**[回归测试](/zh-cn/wiki/regression-testing/)**。
...
[测试脚本](/zh-cn/wiki/test-script/) 作为测试策略的**可执行组件**，将[测试用例](/zh-cn/wiki/test-case/) 转换为测试工具可以自动执行的操作。他们驱动**自动化框架**与测试中的应用程序进行交互，根据[实际结果](/zh-cn/wiki/actual-result/)验证预期结果。通过这样做，[测试脚本](/zh-cn/wiki/test-script/) 提高了测试过程的效率，能够快速执行重复性任务和复杂的[测试场景](/zh-cn/wiki/test-scenario/)，如果手动完成这些任务，这些任务将非常耗时且容易出错。
  通过集成到**持续集成/持续部署 (CI/CD) 管道**，[测试脚本](/zh-cn/wiki/test-script/) 有助于及早检测缺陷和回归，从而有助于 **[左移测试](/zh-cn/wiki/shift-left-testing/) 方法**。这种集成可确保自动化测试频繁且一致地运行，从而为开发团队提供即时反馈。
  ...
  通过利用[测试脚本](/zh-cn/wiki/test-script/)，团队可以更有效地执行**[回归测试](/zh-cn/wiki/regression-testing/)**，确保新的更改不会破坏现有功能。这对于长期维护[软件质量](/zh-cn/wiki/software-quality/) 至关重要。
...
- **[测试脚本](/zh-cn/wiki/test-script/)**
  ...
  本质上，[测试用例](/zh-cn/wiki/test-case/)可以在没有自动化的情况下存在，作为[手动测试](/zh-cn/wiki/manual-testing/)的指南，而[测试脚本](/zh-cn/wiki/test-script/)本质上与自动化相关，并且是[测试用例](/zh-cn/wiki/test-case/)在自动化框架中的实际执行。
...
5. **运行测试**：使用所选自动化工具执行[测试脚本](/zh-cn/wiki/test-script/)。这可以通过命令行、[测试运行者](/zh-cn/wiki/test-runner/) 或持续集成 (CI) 管道启动。
...
- **[selenium](/zh-cn/wiki/selenium/)**：[网络自动化](/zh-cn/wiki/web-automation/) 的开源框架，支持多种语言和浏览器。
...
- **[柏树](/zh-cn/wiki/cypress/)**：在浏览器中运行的基于 JavaScript 的 [端到端测试](/zh-cn/wiki/end-to-end-testing/) 框架。
    ...
    ```
- **JUnit**/**TestNG**：Java 中[单元测试](/zh-cn/wiki/unit-testing/) 的框架，通常与[selenium](/zh-cn/wiki/selenium/) 一起用于自动化。
...
- **RSpec**/**Cucumber**：行为驱动开发工具（[BDD](/zh-cn/wiki/bdd/)），允许以自然语言风格编写测试。
    ...
    ```
- **[邮递员](/zh-cn/wiki/postman/)**：对于[API 测试](/zh-cn/wiki/api-testing/)，能够为 RESTful [API](/zh-cn/wiki/api/) 编写和执行测试。
...
- **检查断言**：验证您的断言是否正确并且正在测试您所期望的内容。不正确的断言可能导致[误报](/zh-cn/wiki/false-positive/) 或否定。
