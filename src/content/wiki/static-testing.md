...
description: "静态测试 (Static testing) 是指在不执行代码的情况下对软件制品进行检查。它涉及 分析 文档和源代码以查找错误，其中可能包括语法错误、代码标准违规和设计问题。此类测试通常通过 人工 努力（如同行评审）和执行静态代码分析的 自动化工具 结合完成…"
section: "S"
related:
  - dynamic-testing
  - inspection
...
- [静态测试 ｜ Static Testing](#静态测试--static-testing)
  - [相关术语](#相关术语)
  - [有关静态测试的问题吗？](#有关静态测试的问题吗)
    ...
    - [技术和方法](#技术和方法)
      ...
      - [静态测试中的走查、检查和审查有什么区别？](#静态测试中的走查检查和审查有什么区别)
      - [什么是静态代码分析？](#什么是静态代码分析)
...
演练、[[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/) 和评论都是 [静态测试](https://inaodeng.com/zh-cn/wiki/static-testing/) 的方法，每种方法都有不同的特征：

- **演练**：这些是非正式会议，软件工件（如代码或设计文档）的作者将其呈现给同行以获取反馈。目标是提供更好的理解并发现异常情况。没有正式的流程；它更像是对材料的引导，通常是为了教育或集思广益。
- **[[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/)**：这些比演练更正式，并且涉及对软件工件的彻底检查。 [[检查](https://inaodeng.com/zh-cn/wiki/inspection/)](https://inaodeng.com/zh-cn/wiki/inspection/) 由主持人（而非作者）领导并遵循规定的流程。该团队包括诸如逐行浏览文档的读者和识别缺陷的检查员等角色。重点是缺陷检测，通常包括后续会议以确保所有问题都得到解决。
...
- **演练**：这些是非正式会议，软件工件（如代码或设计文档）的作者将其呈现给同行以获取反馈。目标是提供更好的理解并发现异常情况。没有正式的流程；它更像是对材料的引导，通常是为了教育或集思广益。
- **[[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/)**：这些比演练更正式，并且涉及对软件工件的彻底检查。 [[检查](https://inaodeng.com/zh-cn/wiki/inspection/)](https://inaodeng.com/zh-cn/wiki/inspection/) 由主持人（而非作者）领导，并遵循规定的流程。该团队包括诸如逐行浏览文档的读者和识别缺陷的检查员等角色。重点是缺陷检测，通常包括后续会议以确保所有问题都得到解决。
- **评论**：这个术语更广泛，可以包含演练和[[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/)。评审可以是正式的或非正式的，包括检查软件工件以发现缺陷、确保符合标准并评估质量。根据组织的流程，审核的形式和结构可能会有很大差异。
...
- 行为 **同行评审**在初始代码提交时。开发人员查看彼此的代码以尽早发现缺陷。这可以包括检查编码标准、命名约定以及对设计原则的遵守。

- 使用 **静态分析工具**扫描源代码是否存在潜在问题，例如安全漏洞、代码异味和可能的错误。这些工具可以自动识别人工审核期间可能遗漏的问题。

- 执行 **模型检查**其中应用形式化方法来验证软件模型的属性，确保系统设计符合指定的要求。

- 参与 **校对**记录拼写错误、语法错误和不一致之处，这些错误可能会在开发过程的后期导致误解。 通过尽早整合这些实践，团队可以在问题变得更加昂贵和耗时之前识别并解决问题。这种主动的方法有助于实现更高效、更可靠的软件开发生命周期。
...
- 参与 **校对**记录拼写错误、语法错误和不一致之处，这些错误可能会在开发过程的后期导致误解。
...
- **静态分析工具**：这些工具分析代码而不执行它。例子包括 `SonarQube` , `Coverity` , `Fortify` , 和 `Checkmarx` 。他们可以检测安全漏洞、代码异味和潜在的错误。

- **IDE 插件**：集成开发环境 (IDE)，例如 `Eclipse` , `Visual Studio` , 和 `IntelliJ IDEA` 通常具有内置的静态分析功能或提供静态代码分析的支持插件。

- **代码审查工具**：诸如此类的工具 `Gerrit` , `Review Board` , `Phabricator` , 和 `Crucible` 通过提供评论和跟踪问题的接口来促进同行代码审查。
...
- **指标和复杂性分析器**：类似的工具 `CodeClimate` 和 `NDepend` 评估代码复杂性、可维护性指数和其他可以指示潜在问题区域的指标。
...
- **主持人（[[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/)）**：在正式的 [[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/) 中，主持人领导审核流程，确保审核系统化，并且所有参与者都做好准备并了解自己的角色。
...
- 实施 **自动静态分析工具**定期扫描代码库，以便持续反馈和早期缺陷检测。
...
- **跟踪和分析指标**例如缺陷密度和修复时间，使用这些数据不断改进您的静态测试方法。
...
[静态测试](https://inaodeng.com/zh-cn/wiki/static-testing/) 的行业标准是确保在不执行代码的情况下评估软件工件的一致且有效的方法的指南和实践。这些标准通常源自正式组织和集体行业经验。
  ...
  **IEEE 1028** 定义了软件审查和审计的标准实践，其中包括 [[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/)、演练、技术审查和管理审查。该标准强调在开发过程的早期识别缺陷和问题。
