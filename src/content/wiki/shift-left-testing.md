---
title: "左移测试 ｜ Shift-left Testing"
description: "左移测试 (Shift-left testing) 是指将测试活动尽早整合到软件开发生命周期 (SDLC) 中的实践。它强调 预防重于检测，通过在问题升级为更大麻烦之前识别并解决它们。这种方法提倡将测试与开发 并行进行，而不是作为一个后续阶段。为了实施左移测试…"
section: "S"
related:
  - automated-testing
  - manual-testing
...
- **服务虚拟化**：WireMock 和 Mountebank 等工具允许模拟服务依赖关系，以便在受控环境中进行测试。 这些工具有助于在开发生命周期的早期嵌入测试，确保更快地识别和解决问题。
...
7. **持续细化**：定期审查和细化测试，以随着代码库的发展保持其有效性和相关性。 通过遵循这些步骤，测试成为开发过程中自然、不可或缺的一部分，从而实现更快的发布和更高的质量。
...
- **实施代码审查**重点关注可测试性，以便在潜在问题传播之前发现它们。

- **利用静态代码分析工具**在运行前检测漏洞和代码气味。

- **将自动化测试集成到 CI/CD 管道中**每次提交都运行，确保立即反馈更改。

- **优先创建有意义且可维护的测试**而不是简单地增加测试数量。

- **培育质量文化**每个人都对最终产品负责，而不仅仅是测试人员。

- **监控测试指标并采取行动**不断改进测试流程和代码质量。
...
- 有一个 **与后期缺陷相关的高成本** ，要么是由于复杂性，要么是对用户体验的影响。

- 该项目如下 **敏捷或DevOps**强调持续集成和交付的实践。

- 有必要 **缩短上市时间**不影响质量。

- 该团队的目标是培养 **品质文化**开发人员从一开始就参与测试活动。

- 应用程序的 **建筑或设计**可以从早期测试（例如微服务、模块化设计）中获益匪浅。

- 有一个重点 **预防措施**而不是对生产中发现的问题进行被动修复。 相反，在以下情况下，右移测试更合适：
...
- **部署后监控**和 **实时问题解决**对业务至关重要。 在实践中，结合左移和右移策略的**平衡方法**通常会产生最佳结果，确保整个 SDLC 的全面覆盖。
...
- **开发者参与度：**增加开发人员对测试的参与，从而更深入地了解代码及其潜在问题。
...
- **重叠的可能性：**可能导致开发人员和测试人员之间的角色混淆或重叠。

- **学习曲线：**开发人员可能需要学习新的测试技能并适应增加的测试职责。

- **资源分配：**可能需要额外的资源来管理早期扩大的测试范围。 **右移测试的优点：**

- **真实世界曝光：**测试是在更接近生产的环境中进行的，提供有关软件在现实条件下如何运行的反馈。

- **用户反馈：**提供在最终版本之前收集用户反馈的机会，这对于改善用户体验非常宝贵。
