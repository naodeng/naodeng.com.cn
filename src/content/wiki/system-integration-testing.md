...
[系统集成测试](/zh-cn/wiki/system-integration-testing/) (SIT) 是一个测试阶段，其中不同的系统组件、模块或服务作为一个组进行集成和测试，以发现集成单元之间交互中的缺陷。它发生在 **[单元测试](/zh-cn/wiki/unit-testing/)** 之后和 **[系统测试](/zh-cn/wiki/system-testing/)** 之前。 SIT 确保集成组件按预期一起运行，并且数据在它们之间正确流动。
  在 SIT 期间，测试人员重点关注模块之间的**接口**和**数据流**。他们验证系统的行为是否符合集成规范，并且可以作为一个内聚单元处理现实场景中的任务。这包括测试 [API](/zh-cn/wiki/api/)、Web 服务、微服务、[database](/zh-cn/wiki/database/) 连接和其他交互点。
...
[单元测试](/zh-cn/wiki/unit-testing/) 和[系统集成测试](/zh-cn/wiki/system-integration-testing/) (SIT) 主要在范围、粒度和目标方面有所不同。
  **[单元测试](/zh-cn/wiki/unit-testing/)** 专注于软件的最小部分，通常是单个函数或方法。它在开发周期的早期进行，旨在确保每个单元能够独立正确运行。 [测试用例](/zh-cn/wiki/test-case/) 由开发人员编写和执行，通常使用 JUnit 或 [NUnit](/zh-cn/wiki/nunit/) 等框架。模拟对象和测试替身通常用于模拟依赖关系的行为。
  相反，**[系统集成测试](/zh-cn/wiki/system-integration-testing/)** 评估集成单元或系统之间的交互。 SIT 检查模块或服务是否按预期协同工作，识别接口缺陷和数据流问题。它在 [单元测试](/zh-cn/wiki/unit-testing/) 之后执行，通常由单独的 QA 团队执行。 SIT需要更复杂的[setup](/zh-cn/wiki/setup/)，包括组件交互的实际环境的配置。
...
- **[自上而下的集成](/zh-cn/wiki/top-down-integration/)** ：集成测试从顶级模块开始，并沿层次结构向下进行，使用
      **存根**
      对于尚未集成的较低级别模块。

- **[自下而上的集成](/zh-cn/wiki/bottom-up-integration/)** ：从最低或最里面的组件开始，然后向上进行，使用
      **司机**
      模拟尚未集成的更高级别的模块。
...
- **[自下而上的集成](/zh-cn/wiki/bottom-up-integration/)** ：从最低或最里面的组件开始，向上进行，使用
      **司机**
      模拟尚未集成的更高级别的模块。
...
- **创建自动[测试套件](/zh-cn/wiki/test-suite/)**
...
- **自动化环境[setup](/zh-cn/wiki/setup/)并拆除**
    确保一致的测试条件和资源的有效利用。

- **集成自动化 SIT 结果**
    进入仪表板和报告工具，以便快速了解系统集成的运行状况。
  通过自动化重复且耗时的任务，工程师可以专注于更复杂的集成场景并确保强大的集成[测试套件](/zh-cn/wiki/test-suite/)。请记住随着系统的发展维护和更新自动化测试，以保持它们的有效性和相关性。
...
- **[非功能测试](/zh-cn/wiki/non-functional-testing/)** ：自动化工具可以模拟数千个虚拟用户进行性能测试，这是手动无法实现的。
  [系统集成测试](/zh-cn/wiki/system-integration-testing/) 自动化工具的缺点：

- **初始投资**：工具和设置测试环境的前期成本很高。
- **维护**：测试脚本需要定期更新以跟上软件更改的步伐，这可能非常耗时。
- **学习曲线**：团队需要时间来学习和适应新工具。
- **复杂[Setup](/zh-cn/wiki/setup/)**：为系统集成测试创建测试环境和数据可能很复杂。
...
- **与 CI/CD 管道集成**
  ...
  通过利用虚拟化，工程师可以确保SIT既**高效**又**代表**实际部署场景，从而增强[集成测试](/zh-cn/wiki/integration-testing/)流程的可靠性。
...
- **审查和重新测试**：修复或更改后，重新测试以确认问题已解决并且没有引入新问题。
  通过遵循这些做法，您可以增强[系统集成测试](/zh-cn/wiki/system-integration-testing/) 的有效性并确保系统组件的集成更加可靠。
...
- **性能瓶颈**：在多系统环境中性能问题可能很难诊断。在集成级别执行 **[性能测试](/zh-cn/wiki/performance-testing/)** 并使用 **分析工具** 来识别瓶颈。
  缓解这些挑战需要结合**战略规划**、**强大的工具**和**适应性流程**。通过主动解决这些问题，[测试自动化](/zh-cn/wiki/test-automation/) 工程师可以确保更高效、更可靠的 SIT 流程。
...
有效地记录 [系统集成测试](/zh-cn/wiki/system-integration-testing/) (SIT) 涉及易于理解和采取行动的清晰、简洁和结构化的信息。以下是记录 SIT 的指南：
  ...
  **[测试数据](/zh-cn/wiki/test-data/)**：记录[测试数据](/zh-cn/wiki/test-data/) 要求，确保其代表生产数据。包括数据[setup](/zh-cn/wiki/setup/) 和清理程序。
  **环境[Setup](/zh-cn/wiki/setup/)**：提供配置[测试环境](/zh-cn/wiki/test-environment/)的说明，包括硬件、软件、网络配置以及任何必要的存根或驱动程序。
...
为大型复杂系统优化[系统集成测试](/zh-cn/wiki/system-integration-testing/) (SIT) 需要一种战略方法来管理所涉及的复杂性和依赖性。 **根据关键业务功能和风险评估确定 [测试用例](/zh-cn/wiki/test-case/)** 的优先级，以重点关注最具影响力的领域。利用**[测试数据](/zh-cn/wiki/test-data/) 管理**工具确保高质量、相关的[测试数据](/zh-cn/wiki/test-data/) 可用，从而减少花在数据[setup](/zh-cn/wiki/setup/) 和维护上的时间。
