...
[网络自动化](/zh-cn/wiki/web-automation/) 是指使用软件模拟用户与网络浏览器和网络应用程序交互的过程。它涉及执行脚本来执行诸如单击按钮、将数据输入表单、浏览网站和提取信息等任务。这样做通常是为了测试 Web 应用程序的功能和性能，以确保它们在不同的浏览器和设备上按预期工作。
  **[网络自动化](/zh-cn/wiki/web-automation/)** 是通过专门的工具和框架实现的，这些工具和框架根据 HTML 结构与 Web 元素进行交互。这些工具可以以编程方式控制浏览器，模仿用户操作，无需手动干预。它们可以集成到 [自动化测试](/zh-cn/wiki/automated-testing/) 和部署的持续集成管道中。
...
[网络自动化](/zh-cn/wiki/web-automation/) 至关重要，原因如下：
...
[网络自动化](/zh-cn/wiki/web-automation/) 的关键组件包括：
...
- **[API](/zh-cn/wiki/api/)** ：与网页交互； WebDriver API、Puppeteer API。
...
- **改进[测试覆盖率](/zh-cn/wiki/test-coverage/)**：自动化允许更广泛的测试覆盖范围，包括可能耗时或难以手动执行的复杂场景。
...
[网络自动化](/zh-cn/wiki/web-automation/) 虽然功能强大，但也有自己的一系列**挑战**：

- **[可维护性](/zh-cn/wiki/maintainability/)** ：随着频繁的 UI 更改，自动化测试可能会变得脆弱，从而导致高昂的维护成本。
...
[网络自动化](/zh-cn/wiki/web-automation/) 的热门工具包括：
...
- **UFT（统一[功能测试](/zh-cn/wiki/functional-testing/)）**：以前称为 QTP，UFT 为功能性和[回归测试](/zh-cn/wiki/regression-testing/) 提供了全面的[测试自动化](/zh-cn/wiki/test-automation/) 解决方案以及可视化脚本界面。
...
由于 JavaScript 在 Web 浏览器中的本机支持以及与网页元素交互的能力，因此 JavaScript 在 [网络自动化](/zh-cn/wiki/web-automation/) 中发挥着**核心作用**。它是网络的**脚本语言**，使自动化工具能够执行以下任务：
...
设置 [网络自动化](/zh-cn/wiki/web-automation/) 测试涉及几个关键步骤：
...
- **[跨浏览器测试](/zh-cn/wiki/cross-browser-testing/)**：确保 Web 应用程序在不同浏览器和版本上正常运行，通常使用基于云的工具来提供对多个浏览器环境的访问。
- **持续集成 (CI) 和持续部署 (CD)**：将 [网络自动化](/zh-cn/wiki/web-automation/) 测试集成到 CI/CD 管道中，以便及早发现问题并自信地进行部署。
- **[视觉回归测试](/zh-cn/wiki/visual-regression-testing/)**：将网页的视觉效果与基线进行比较，以检测意外的更改。
- **[API 测试](/zh-cn/wiki/api-testing/)**：验证应用程序的[API](/zh-cn/wiki/api/) 层的功能、可靠性、性能和安全性，该层通常是 Web 应用程序的关键组件。
...
- **持续集成 (CI) 和持续部署 (CD)**：将 [网络自动化](/zh-cn/wiki/web-automation/) 测试集成到 CI/CD 管道中，以便及早发现问题并自信地进行部署。
- **[视觉回归测试](/zh-cn/wiki/visual-regression-testing/)**：将网页的视觉效果与基线进行比较，以检测意外的更改。
- **[API 测试](/zh-cn/wiki/api-testing/)**：验证应用程序 [API](/zh-cn/wiki/api/) 层的功能、可靠性、性能和安全性，该层通常是 Web 应用程序的关键组件。
- **[负载测试](/zh-cn/wiki/load-testing/)**：模拟大量用户来测试应用程序在预期负载和峰值负载条件下的性能。
- **[辅助功能测试](/zh-cn/wiki/accessibility-testing/)**：确保 Web 应用程序可供残疾人使用，符合 WCAG 等标准。
- **移动[网页测试](/zh-cn/wiki/web-testing/)**：在移动设备或模拟器/仿真器上测试 Web 应用程序，以确保移动平台上的功能和可用性。
...
- **[可维护性](/zh-cn/wiki/maintainability/)** ：编写干净、可读且可维护的代码。使用页面对象模型将测试逻辑与特定于页面的代码分开。
...
- **[测试环境](/zh-cn/wiki/test-environment/)** ：确保测试在稳定一致的环境中运行，以避免不稳定的结果。
...
在 [网络自动化](/zh-cn/wiki/web-automation/) 中处理 **CAPTCHA** 或 **双因素身份验证 (2FA)** 可能具有挑战性，因为它们的目的是将人类用户与机器人区分开来。以下是一些策略：

1. **验证码绕过选项**：
- **[测试环境](/zh-cn/wiki/test-environment/)** ：与开发团队合作在测试环境中禁用验证码。
- **[API](/zh-cn/wiki/api/) Key** ：一些 CAPTCHA 提供商提供测试 API 密钥，这些密钥始终返回可预测的响应。
...
为了确保 [网络自动化](/zh-cn/wiki/web-automation/) 测试**可靠且稳健**，请遵循以下准则：

- **可重用性设计**：使用可重用组件创建模块化脚本，以最大限度地减少冗余并促进维护。
- **实现显式等待**：使用显式等待来处理异步操作并确保元素在交互之前可用。
- **使用[页面对象模型](/zh-cn/wiki/page-object-model/) (POM)** ：将页面详细信息抽象为对象，将测试逻辑与UI结构分开，增强可维护性。
...
通过引入**自适应学习**和**预测分析**，人工智能在增强[网络自动化](/zh-cn/wiki/web-automation/)方面发挥着**关键作用**。它有助于创建更加**智能**和**弹性** [测试脚本](/zh-cn/wiki/test-script/)，可以适应 Web 应用程序 UI 或功能的变化，而无需持续的人工干预。
  [网络自动化](/zh-cn/wiki/web-automation/) 中人工智能的关键应用之一是**自我修复测试**。人工智能算法可以检测到测试何时由于应用程序中的微小变化而中断，并且可以**自动更新**选择器或交互模式以保持测试通过，从而减少维护开销。
