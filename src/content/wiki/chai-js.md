...
[Chai.js](/zh-cn/wiki/chai-js/) 是一个**行为驱动开发 ([BDD](/zh-cn/wiki/bdd/))** / **[test-driven development](/zh-cn/wiki/test-driven-development/) (TDD)** 断言库，适用于 [Node.js](/zh-cn/wiki/node-js/) 和浏览器，可以与任何 JavaScript 测试框架完美搭配。它为开发人员提供了一组丰富的、可读且富有表现力的断言。
  [Chai.js](/zh-cn/wiki/chai-js/) 提供三种不同风格的断言：**should**、**expect** 和 **assert**。每种样式都有自己的语法，允许开发人员选择最适合他们的偏好或项目需求的样式。
...
```
  ...
  assert.exists(foo);
```
[Chai.js](/zh-cn/wiki/chai-js/) 断言可以处理各种测试条件，包括但不限于属性值、深度相等检查和引发的异常。它还支持使用回调和承诺来测试异步操作。
  该库可通过插件进行扩展，可以添加新的断言和测试条件以满足特定的测试要求。这种可扩展性使[Chai.js](/zh-cn/wiki/chai-js/)能够适应各种[use cases](/zh-cn/wiki/use-case/)并与其他工具和库集成。
  [Chai.js](/zh-cn/wiki/chai-js/) 通常与其他测试工具（例如 Mocha、[Jest](/zh-cn/wiki/jest/) 或 Karma）结合使用，提供完整的测试解决方案。它通过 npm 安装，可以轻松集成到任何 JavaScript 项目中，使其成为希望增强测试能力的开发人员的便捷选择。

#### 为什么在测试中使用 Chai.js？

[Chai.js](/zh-cn/wiki/chai-js/) 用于测试主要是因为其在编写断言时的**灵活性**和**可读性**。它允许开发人员编写富有表现力且易于维护的测试。通过提供**行为驱动开发 ([BDD](/zh-cn/wiki/bdd/))** 和 **[Test-Driven Development](/zh-cn/wiki/test-driven-development/) (TDD)** 断言样式，它可以满足不同的偏好，并且可以轻松与 Mocha 或 [Jest](/zh-cn/wiki/jest/) 等各种测试框架集成。
  [Chai.js](/zh-cn/wiki/chai-js/) 的使用增强了**调试体验**，因为它提供了信息丰富的错误消息，其中详细说明了断言失败时的预期与 [actual result](/zh-cn/wiki/actual-result/) 的情况。此功能显着减少了确定测试失败原因所花费的时间。
  此外，[Chai.js](/zh-cn/wiki/chai-js/) 支持**可链接断言**，这使得能够以可读的方式组合复杂的条件。这种链接模仿自然语言，使新开发人员或非技术利益相关者更容易理解测试。
  [Chai.js](/zh-cn/wiki/chai-js/) 通过插件的可扩展性允许定制和扩展其核心功能。这意味着团队可以使库适应他们的特定测试需求，而无需等待核心包提供这些功能。
  总之，选择 [Chai.js](/zh-cn/wiki/chai-js/) 是因为它的**表达语法**、**有用的错误消息**、**与其他工具的兼容性**和**可扩展性**，所有这些都有助于提供更高效、更愉快的测试体验。
...
- **[BDD](/zh-cn/wiki/bdd/)/TDD 断言样式**：Chai 提供两种主要的断言样式：**行为驱动开发 ([BDD](/zh-cn/wiki/bdd/))** 使用 `expect` 或 `should`，以及 **[Test-Driven Development](/zh-cn/wiki/test-driven-development/) (TDD)** 使用 `assert`。这使得开发人员可以选择最适合他们的测试理念的风格，或者在他们的[test suite](/zh-cn/wiki/test-suite/) 中混合搭配。
  ...
  - **属性测试**：它允许轻松检查对象属性，这对于测试 [API](/zh-cn/wiki/api/) 响应和复杂的数据结构非常有用。
  - **相等和比较**：Chai 提供了一套全面的相等和比较断言，包括深度相等检查和大于/小于关系的断言。
  - **错误处理**：包含错误处理断言，允许开发人员断言某些函数在特定条件下抛出预期错误。
  - **插件架构**：Chai 通过插件的可扩展性意味着它可以适应各种 [use cases](/zh-cn/wiki/use-case/) 并与其他库和工具集成。
  - **异步支持**：它内置了对测试异步代码的支持，包括 Promise，这对于现代 JavaScript 开发至关重要。
  - **自定义消息**：开发人员可以为断言提供自定义错误消息，这可以使调试失败的测试更容易、信息更丰富。
  - **跨平台**：[Chai.js](/zh-cn/wiki/chai-js/) 可在 [Node.js](/zh-cn/wiki/node-js/) 和浏览器环境中工作，使其适用于各种 JavaScript 项目。
  - **[BDD](/zh-cn/wiki/bdd/)/TDD 断言样式**：Chai 提供两种主要的断言样式：**行为驱动开发 ([BDD](/zh-cn/wiki/bdd/))** 与 `expect` 或 `should`，以及 **[Test-Driven Development](/zh-cn/wiki/test-driven-development/) (TDD)** 与 `assert`。这使得开发人员可以选择最适合他们的测试理念的风格，或者在他们的[test suite](/zh-cn/wiki/test-suite/) 中混合搭配。
  - **可链接语言**：可以使用自然语言结构将断言链接在一起以形成可读的语句。这提高了测试的可读性并使编写断言更加直观。
  - **类型检查**：Chai 包含用于类型检查的断言，例如 `expect(value).to.be.a('string')`，它通过确保值属于预期类型来增强测试的稳健性。
  - **属性测试**：它允许轻松检查对象属性，这对于测试 [API](/zh-cn/wiki/api/) 响应和复杂的数据结构非常有用。
  - **相等和比较**：Chai 提供了一套全面的相等和比较断言，包括深度相等检查和大于/小于关系的断言。
...
要安装 **[Chai.js](/zh-cn/wiki/chai-js/)**，请确保已安装 **[Node.js](/zh-cn/wiki/node-js/)** 和 **npm**（节点包管理器）。打开终端或命令提示符并导航到您的项目目录。运行以下命令：

```
  npm install chai --save-dev
```
此命令将 [Chai.js](/zh-cn/wiki/chai-js/) 作为开发依赖项安装，并将其添加到 `package.json` 文件中。安装完成后，您可以使用以下命令将 Chai 导入到测试文件中：
  对于 CommonJS 模块：
...
```
然后，您可以使用 Chai 的 `expect`、`should` 或 `assert` 接口来编写测试。如果您还没有安装，请记住还要安装[test runner](/zh-cn/wiki/test-runner/)，如 **Mocha** 或 **[Jest](/zh-cn/wiki/jest/)**，因为 Chai 是一个断言库，本身不提供测试框架。

#### 使用 Chai.js 有哪些先决条件？

要使用 **[Chai.js](/zh-cn/wiki/chai-js/)**，请确保满足以下先决条件：
...
- **[Node.js](/zh-cn/wiki/node-js/)** ：Chai 是一个 Node.js 库，因此您需要在系统上安装 Node.js。该版本应与您计划使用的 Chai 版本兼容。
  ...
  - **项目[环境搭建](/zh-cn/wiki/setup/)** ：您的项目应该使用 `package.json` 文件（如果您使用的是 NPM 或 Yarn）。该文件跟踪与您的项目相关的依赖项和脚本。
...
要为您的项目设置 [Chai.js](/zh-cn/wiki/chai-js/)，请按照以下步骤操作：
...
5. **使用您选择的[test runner](/zh-cn/wiki/test-runner/)（例如，Mocha、[Jest](/zh-cn/wiki/jest/)）运行测试**。
  6. 或者，如果需要，**使用其他设置配置 Chai**，例如使用插件或向断言添加自定义消息。
  7. **将 Chai 与其他工具集成**，例如 [test runners](/zh-cn/wiki/test-runner/) 或测试环境所需的模拟库。
  请记住**检查项目的依赖项**以确保它们与您正在使用的 Chai 版本兼容。保持 Chai 和所有插件更新，以受益于最新功能和[bug](/zh-cn/wiki/bug/) 修复。
...
5. **使用您选择的[test runner](/zh-cn/wiki/test-runner/)（例如，Mocha、[Jest](/zh-cn/wiki/jest/)）运行测试**。
  6. 或者，如果需要，**使用其他设置配置 Chai**，例如使用插件或向断言添加自定义消息。
  7. **将 Chai 与其他工具集成**，如 [test runners](/zh-cn/wiki/test-runner/) 或测试环境所需的模拟库。
...
[Chai.js](/zh-cn/wiki/chai-js/) 中的 **断言** 是计算测试中的表达式或值的语句。它检查该表达式或值是否满足某些条件，如果不满足，则断言将失败，从而导致测试失败。断言是[test suites](/zh-cn/wiki/test-suite/) 的核心组件，因为它们验证被测代码的行为。
...
要在 [Chai.js](/zh-cn/wiki/chai-js/) 中编写基本断言，您可以使用其任何接口：`expect`、`should` 或 `assert`。以下是使用 `expect` 接口的示例：
...
```
在此示例中，`expect` 函数用于对`[1, 2, 3].indexOf(4)` 的结果进行断言。 `.to.equal(-1)` 链是实际的断言，表明[expected result](/zh-cn/wiki/expected-result/) 应该是`-1`。
...
[Chai.js](/zh-cn/wiki/chai-js/) 提供三种断言样式：**should**、**expect** 和 **assert**。每种风格都提供了多种断言来测试不同的条件：
  ### 应该和期望
  这些[BDD](/zh-cn/wiki/bdd/)（行为驱动开发）样式在功能上相似，但在语法上有所不同。它们提供了一种可链接的语言来构造断言。
...
- **.exist** ：断言非空且非未定义。
  - **.empty** ：断言空数组、字符串或对象。
  ### 断言
  TDD ([Test-Driven Development](/zh-cn/wiki/test-driven-development/)) 风格使用更传统的断言方法，无需可链接语言。
...
要断言函数在 [Chai.js](/zh-cn/wiki/chai-js/) 中引发错误，您可以使用 `expect` 或 `should` 接口中的 `throw` 或 `throws` 方法。以下是使用这两个界面执行此操作的方法：
  **使用`expect`接口：**
...
要断言[Chai.js](/zh-cn/wiki/chai-js/) 中的深度相等，请使用`.deep` 链，后跟`.equal` 或`.eql` 断言。这将在考虑所有嵌套属性的情况下，在目标和预期对象之间执行深度比较。
  以下是使用 `expect` 接口的示例：
...
- `chai-http` ：启用 HTTP 断言，使测试 Web 服务变得容易。
  ...
  创建**自定义 [Chai.js](/zh-cn/wiki/chai-js/) 插件**涉及定义导出函数的模块。此函数应接受 Chai 实例，并应使用 Chai 的 [API](/zh-cn/wiki/api/) 添加新方法或属性：
...
要使用 [Chai.js](/zh-cn/wiki/chai-js/) 插件，请按照以下步骤操作：
...
[Chai.js](/zh-cn/wiki/chai-js/) 拥有丰富的插件生态系统，可扩展其核心功能。以下是一些受欢迎的：
...
创建您自己的 [Chai.js](/zh-cn/wiki/chai-js/) 插件涉及使用新的断言或行为来扩展 Chai。请按照下列步骤操作：
...
1. **彻底测试您的插件**。使用 Mocha 或其他测试框架创建 [test cases](/zh-cn/wiki/test-case/) 以确保您的断言按预期工作。
...
```
  ...
  const expect = chai.expect;
```
在处理 Promise 时，您可以将带有断言的 Promise 返回给 [test runner](/zh-cn/wiki/test-runner/)，它将等待 Promise 解析或拒绝：
...
将 **[Chai.js](/zh-cn/wiki/chai-js/)** 与 Promise 一起使用涉及利用 **chai-as-promised** 插件，该插件扩展了 Chai 以实现流畅的 Promise 断言。首先，确保安装 **chai-as-promised** 并将其与 Chai 集成：
...
```
请记住从 [test case](/zh-cn/wiki/test-case/) 返回 Promise 或使用 `done` 回调来确保测试等待 Promise 解析：
...
.should` 接口的工作原理是向 `Object.prototype` 添加一个 getter，返回 `Should` 断言对象。然后可以使用该对象将进一步的断言链接到正在测试的值。请务必注意，使用 `.should` 会修改 `Object.prototype`，如果您的应用程序依赖于对象的枚举属性，这可能会导致意外行为。
  使用`.should` 的断言在失败时抛出`AssertionError`，然后[test runner](/zh-cn/wiki/test-runner/) 可以捕获该断言以报告失败。 `.should` 接口支持与 Chai 的 `expect` 和 `assert` 接口相同的所有断言，提供丰富的断言集，如 `.equal`、`.deep.equal`、`.have.property` 等。
...
自定义[Chai.js](/zh-cn/wiki/chai-js/)断言错误消息可以增强测试结果的可读性和清晰度。要自定义错误消息，请使用 Chai 提供的 `.message` 可链接方法。此方法允许您指定断言失败时将显示的自定义消息。
  以下是使用 `expect` 接口的示例：
...
```
`.should` 接口的工作原理是向 `Object.prototype` 添加一个 getter，返回 `Should` 断言对象。然后可以使用该对象将进一步的断言链接到正在测试的值。请务必注意，使用 `.should` 会修改 `Object.prototype`，如果您的应用程序依赖于对象的枚举属性，这可能会导致意外行为。
  使用`.should` 的断言在失败时抛出`AssertionError`，然后[测试运行者](/zh-cn/wiki/test-runner/) 可以捕获该断言以报告失败。 `.should` 接口支持与 Chai 的 `expect` 和 `assert` 接口相同的所有断言，提供丰富的断言集，如 `.equal`、`.deep.equal`、`.have.property` 等。
  使用 `.should` 时，您还可以利用 Chai 的可链接语言来增强测试的可读性：
