...
5. **重复**
  ...
  对于自动化，开发脚本来验证新模块集成和回归测试以确保现有功能不受影响。 Jenkins、JUnit 或 [selenium](/zh-cn/wiki/selenium/) 等工具可用于自动化这些测试，具体取决于技术堆栈和项目要求。
...
- **三明治测试（混合[集成测试](/zh-cn/wiki/integration-testing/)）**：结合了自上而下和自下而上的方法。首先使用存根和驱动程序测试中层模块，然后逐步向顶部和底部集成。
  每种类型的[增量测试](/zh-cn/wiki/incremental-testing/) 都针对软件的不同方面，可以根据项目的特定需求（例如架构、组件的关键性和资源可用性）进行选择。
...
3. **系统集成**：系统的较大部分经过集成和测试，以确保它们按预期协同工作。
  ...
  自下而上[增量测试](/zh-cn/wiki/incremental-testing/) 通常与**自上而下测试**形成对比，后者的过程从顶层模块开始并向下进行。两者之间的选择取决于项目的具体环境，例如系统的设计和组件之间的依赖关系。
...
- **[测试存根](/zh-cn/wiki/test-stub/) 和驱动程序的可用性**：
    **自上而下**
    需要尚未开发的较低级别模块的存根。
    **自下而上**
...
9. **最终测试**：对整个软件进行系统级测试以验证整体功能。
  在此过程中，**持续反馈**对于及早发现问题至关重要。 [测试自动化](/zh-cn/wiki/test-automation/) 工程师应该使用**自动回归测试**来保持效率。 **自上而下**和**自下而上**方法之间的选择取决于系统架构和组件的关键性。版本控制系统、持续集成平台和[测试自动化](/zh-cn/wiki/test-automation/) 框架等**工具**支持[增量测试](/zh-cn/wiki/incremental-testing/) 流程。
...
10. **文档**：保持测试文档更新，以反映系统和测试工作的当前状态。
  通过遵循这些步骤，可以有效地实施[增量测试](/zh-cn/wiki/incremental-testing/)，确保应用程序的每个部分都经过彻底的测试和顺利集成，从而形成更加可靠和可维护的软件产品。
...
- **[回归测试](/zh-cn/wiki/regression-testing/)**
  ...
  在这些场景中，可以战略性地应用[增量测试](/zh-cn/wiki/incremental-testing/)来最大化[测试覆盖率](/zh-cn/wiki/test-coverage/)、管理复杂性并保持稳定的开发和测试节奏。它与强调持续集成和交付的**敏捷**和**DevOps**实践非常吻合。 [增量测试](/zh-cn/wiki/incremental-testing/) 适用于 **自上而下** 和 **自下而上** 方法，具体取决于项目要求和系统组件之间的依赖关系。
...
1. **识别新的[测试用例](/zh-cn/wiki/test-case/)**
    当前迭代的功能。

2. **更新现有[测试用例](/zh-cn/wiki/test-case/)**
    以适应变化。
...
4. **分析测试结果**
    并解决发现的任何缺陷。
  [测试自动化](/zh-cn/wiki/test-automation/) 工程师利用**持续集成 (CI) 工具**来自动执行这些测试，提供有关应用程序运行状况的快速反馈。这种反馈循环对于在快节奏的敏捷环境中保持质量至关重要。
  敏捷中的[增量测试](/zh-cn/wiki/incremental-testing/) 是关于**建立在坚实的基础上**，其中每个[迭代](/zh-cn/wiki/iteration/) 的成功都取决于先前增量的稳健性。这是一项协作工作，需要开发人员、测试人员和整个敏捷团队密切合作，以确保每个增量都满足质量标准，然后再增加系统的复杂性。
