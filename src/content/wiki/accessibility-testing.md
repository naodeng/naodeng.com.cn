...
相关术语：
[Usability Testing 易用性测试](/zh-cn/wiki/usability-testing/)
...
**测试技巧** 既包括自动化，也包括[手工测试](/zh-cn/wiki/manual-testing/)方法。自动化工具可扫描特定类型的问题，如缺失的 alt 文本或错误的 ARIA 角色，而[手工测试](/zh-cn/wiki/manual-testing/)可能涉及使用屏幕阅读器或仅通过键盘浏览应用。
...
- **[手工测试](/zh-cn/wiki/manual-testing/)**：结合[自动化测试](/zh-cn/wiki/automated-testing/)进行手动检查，特别是对于主观标准，如导航和理解的便利性。
...
Section 508 是 1973 年康复法案的一部分，要求联邦机构使其电子和信息技术（EIT）对残障人士可访问。在软件[测试自动化](/zh-cn/wiki/test-automation/)的情境中，Section 508 合规性意味着确保应用程序和网站可供具有各种残障的个体使用，包括视觉、听觉、身体、言语、认知、语言、学习和神经系统残障。
...
自动化工具可以帮助识别一些 Section 508 合规性问题，但[手工测试](/zh-cn/wiki/manual-testing/)也是必要的，以确保完全的可访问性。[测试自动化](/zh-cn/wiki/test-automation/)工程师应将自动和手动的可访问性检查集成到其测试策略中，以涵盖 Section 508 中概述的广泛要求。这种集成有助于创建一个包容性的用户体验，并减轻与不合规相关的法律和声誉风险。
...
**[测试自动化](/zh-cn/wiki/test-automation/)工程师**应将 ARIA 角色验证整合到其测试套件中，以确保 Web 应用具有可访问性并为辅助技术提供必要的上下文。
...
- **[Lighthouse](/zh-cn/wiki/lighthouse/)**：Lighthouse 是一个用于提高网页质量的开源自动化工具，它有性能、可访问性、渐进式 Web 应用等审核。
...
- **手动代码[检查](/zh-cn/wiki/inspection/)**：审查 HTML/CSS 代码，检查语义结构、标题、标签和辅助技术所依赖的角色的正确使用。
...
这些手动技巧与[自动化测试](/zh-cn/wiki/automated-testing/)相辅相成，弥补了需要人类判断和视角的方面，这些方面通常被自动化工具所遗漏。

### 如何在无障碍测试中使用自动化工具？

自动化工具通过迅速扫描网页和应用程序，寻找常见的可访问性问题，从而简化了无障碍测试。它们可以集成到 CI/CD 流程中，以确保与可访问性标准的**持续合规**。像**axe-core**、**WAVE**或**[Lighthouse](/zh-cn/wiki/lighthouse/)**这样的工具提供了[APIs](/zh-cn/wiki/api/)和插件，可与测试框架（如[Selenium](/zh-cn/wiki/selenium/)、[Jest](/zh-cn/wiki/jest/)或[Cypress](/zh-cn/wiki/cypress/)）集成。
...
自动化工具不能取代[手工测试](/zh-cn/wiki/manual-testing/)或与残障人士进行用户测试，但它们是识别和缓解可访问性障碍的**有价值的第一步**。它们有助于保持可访问性的基线水平，并减少需要手动审查的问题数量。

### 自动无障碍测试工具有哪些局限性？

自动化无障碍测试工具存在一些限制：

- **[误报](/zh-cn/wiki/false-positive/)或漏报**：工具可能报告并非真正存在的问题（误报），或者漏掉真实的问题（漏报）。
...
为了缓解这些限制，应将[自动化测试](/zh-cn/wiki/automated-testing/)与**[手工测试](/zh-cn/wiki/manual-testing/)**和与残疾人士的**用户测试**相结合。这种方法提供了更准确、全面的可访问性评估。
...
请记住，尽管自动化工具可以捕捉许多可访问性问题，但它们无法检测到所有残障人士的用户体验的微妙之处。与真实用户或无障碍专家进行的**[手工测试](/zh-cn/wiki/manual-testing/)**对于全面评估至关重要。
...
**在测试阶段**，将无障碍性纳入[测试用例](/zh-cn/wiki/test-case/)中，并执行自动化和手动测试。自动化测试可以捕获各种问题，但[手工测试](/zh-cn/wiki/manual-testing/)对于从人的角度评估可用性至关重要。

**在部署阶段**，执行最终的无障碍性审查和验证，以确保没有引入新问题。

**在部署后**，建立与用户的反馈循环，以捕获可能被忽略的无障碍问题，并对用户需求做出响应。定期更新[测试套件](/zh-cn/wiki/test-suite/)和工具，以
...
为了在软件[测试自动化](/zh-cn/wiki/test-automation/)中确保持续的无障碍性合规性：
...
这些问题可以通过使用自动化工具和[手动测试](/zh-cn/wiki/manual-testing/)的组合来识别，以确保全面评估无障碍性。
