...
### 相关术语：
- [易用性测试](/zh-cn/wiki/usability-testing/)
...
技术上涉及**自动化测试**和**[手工测试](/zh-cn/wiki/manual-testing/)**方法。自动化工具可以扫描某些类型的问题，如缺失的替代文本 (alt text) 或不正确的 ARIA 角色，而手工测试可能包括使用屏幕阅读器操作应用或仅通过键盘进行导航。
...
- **[手工测试](/zh-cn/wiki/manual-testing/)**：通过手动检查来补充**[自动化测试](/zh-cn/wiki/automated-testing/)**，特别是针对导航便利性和理解度等主观标准。
...
Section 508 是 1973 年《康复法案》的一部分，要求联邦机构使其电子和信息技术 (EIT) 对残障人士无障碍。在**[测试自动化](/zh-cn/wiki/test-automation/)**的背景下，Section 508 合规性意味着确保应用程序和网站可供具有各种障碍的个人使用。
...
自动化工具可以协助识别某些问题，但**[手工测试](/zh-cn/wiki/manual-testing/)**对于全面确保无障碍性也是必要的。**[测试自动化](/zh-cn/wiki/test-automation/)**工程师应将自动化和手动无障碍检查都整合到其测试策略中。
...
- **[Lighthouse](/zh-cn/wiki/lighthouse/)**：一种开源的自动化工具，用于提高网页质量。它具有针对性能、无障碍性、渐进式 Web 应用等的审核。
...
- **代码走查 ([Inspection](/zh-cn/wiki/inspection/))**：审查 HTML/CSS 代码的语义结构、标题、标签和角色。
- **缩放和放大**：在不同的缩放级别下测试应用，确保内容清晰且功能正常。
- **内容可读性**：评估语言是否清晰简洁，这有利于认知障碍用户。
- **焦点管理**：确保焦点顺序符合逻辑且可见。
- **残障人士参与的用户测试**：邀请残障用户参与测试过程，获取直接反馈。

这些手动方法补充了**[自动化测试](/zh-cn/wiki/automated-testing/)**，涵盖了需要人类判断和视角的方面。

- **自动化工具如何用于无障碍测试？**

自动化工具通过快速扫描网页来发现常见的无障碍问题。它们可以集成到 CI/CD 流水线中，以确保**持续合规**。诸如 **axe-core**、**WAVE** 或 **[Lighthouse](/zh-cn/wiki/lighthouse/)** 之类的工具提供 **[API](/zh-cn/wiki/api/)** 和插件，可与 **[Selenium](/zh-cn/wiki/selenium/)**、**[Jest](/zh-cn/wiki/jest/)** 或 **[Cypress](/zh-cn/wiki/cypress/)** 等测试框架集成。
...
自动化工具不能替代**[手工测试](/zh-cn/wiki/manual-testing/)**或用户测试，但它们是发现障碍的**宝贵第一步**。

- **自动化无障碍测试工具有哪些局限性？**

自动化无障碍测试工具有几个局限性：
- **误报/漏报 ([False Positives](/zh-cn/wiki/false-positive/))**：工具可能会报告并非实际障碍的问题，或漏掉真实问题。
- **语境理解**：它们缺乏理解语境和含义的能力。
...
为了减轻这些局限性，应将**[自动化测试](/zh-cn/wiki/automated-testing/)**与**手工测试**和针对残障人士的**用户测试**结合起来。
...
在测试环境中加入**辅助技术**和**用户偏好**。请记住，虽然自动化工具可以捕获许多问题，但无法检测到用户体验的所有细微差别。**[手工测试](/zh-cn/wiki/manual-testing/)**对于综合评估至关重要。
...
- **培训团队**了解无障碍原则。
- **创建检查清单**（基于 WCAG）。
- **结合使用自动化和[手工测试](/zh-cn/wiki/manual-testing/)**。
- **自动化重复任务**（如对比度检查）。
- **开展用户测试**。
- **定期评审和更新测试**。
- **完整记录无障碍问题**。
...
- **测试阶段**：在**[测试用例](/zh-cn/wiki/test-case/)**中包含无障碍性，执行自动化和**[手工测试](/zh-cn/wiki/manual-testing/)**。
- **部署阶段**：执行最终审查和验证。
- **发布后**：建立用户反馈循环，并定期更新**[测试套件](/zh-cn/wiki/test-suite/)**。

- **如何确保持续的无障碍合规性？**

要在**[测试自动化](/zh-cn/wiki/test-automation/)**中确保持续合规：
- **将无障碍检查集成到常规测试套件中**。
