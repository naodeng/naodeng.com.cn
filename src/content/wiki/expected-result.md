...
在[软件测试](/zh-cn/wiki/software-testing/) 中，**[预期结果](/zh-cn/wiki/expected-result/)** 是[测试用例](/zh-cn/wiki/test-case/) 在指定条件下执行时应产生的预定义输出或行为。它源自软件的需求或设计规范，并作为测试过程中验证系统实际响应正确性的基准。
  [预期结果](/zh-cn/wiki/expected-result/) 对于自动化测试至关重要，因为它们使自动化框架能够在无需人工干预的情况下确定通过或失败的结果。运行测试时，自动化工具会捕获 **[实际结果](/zh-cn/wiki/actual-result/)** 并将其与 [预期结果](/zh-cn/wiki/expected-result/) 进行比较。匹配确认系统按预期运行，而不匹配可能表明测试本身存在缺陷或问题。
...
定义 **[预期结果](/zh-cn/wiki/expected-result/)** 在 [软件测试](/zh-cn/wiki/software-testing/) 中至关重要，因为它充当验证软件行为正确性的基准。如果没有明确的预期结果，测试人员无法最终确定测试是否通过或失败，从而导致模糊性和潜在的缺陷监督。它确保软件的功能符合要求和设计规范，为[测试执行](/zh-cn/wiki/test-execution/)期间的比较提供具体基础。
  明确定义的 [预期结果](/zh-cn/wiki/expected-result/) 还允许执行断言检查，以编程方式比较预期结果和实际结果，从而促进自动化测试验证。这种比较对于持续集成和交付管道至关重要，其中自动化测试必须可靠地评估软件的稳定性，而无需人工干预。
  此外，当 [预期结果](/zh-cn/wiki/expected-result/) 没有明确定义时，可能会导致测试工作不一致，不同的测试人员可能对成功测试的构成有不同的解释。这种不一致可能会导致 [测试覆盖率](/zh-cn/wiki/test-coverage/) 中的差距以及对软件质量的错误信心。
  总之，[预期结果](/zh-cn/wiki/expected-result/) 的定义是结构化且有效的测试过程的基础，确保测试可重复、结果可解释以及软件满足其预期行为。

#### “预期结果”对整个测试过程有何贡献？

*预期结果*对于指导 **[测试自动化](/zh-cn/wiki/test-automation/) 流程**至关重要。它充当根据预定义标准验证软件行为的**基准**。通过明确的预期结果，自动化测试可以**立即**并**准确**确定[测试用例](/zh-cn/wiki/test-case/) 是否通过或失败，从而简化测试周期。
...
如果 **[预期结果](/zh-cn/wiki/expected-result/)** 定义不正确，可能会出现几个问题：

- **[误报](/zh-cn/wiki/false-positive/)/Negatives** ：测试可能在应该失败时通过（误报），或者在应该通过时失败（误报），从而导致对软件质量的错误假设。
...
创建 **[预期结果](/zh-cn/wiki/expected-result/)** 涉及分析软件的**要求**和**规范**，以确定 [测试用例](/zh-cn/wiki/test-case/) 的正确结果。这是一个分步方法：

1. **审查要求**：彻底检查功能性和非[功能需求](/zh-cn/wiki/functional-requirements/) 或用户故事，以了解预期行为。
...
定义 **[预期结果](/zh-cn/wiki/expected-result/)** 的责任通常落在 **[测试用例](/zh-cn/wiki/test-case/) 作者**身上，他通常是**测试分析师**或**软件测试人员**。在某些情况下，这可能还涉及与**产品所有者**、**业务分析师**或**开发人员**的协作，以确保与需求和功能保持一致。 [测试用例](/zh-cn/wiki/test-case/) 作者必须清楚地了解系统的行为以及测试正在验证的特定需求或用户故事。他们利用这些知识来阐明执行测试时的正确结果应该是什么。
  在**敏捷环境**中，定义[预期结果](/zh-cn/wiki/expected-result/) 是**团队的努力**，**开发人员**、**测试人员**和**业务利益相关者**共同努力明确验收标准。这种协作方法确保每个人都对该功能的预期行为有共同的理解。
  对于**复杂系统**或需要领域专业知识时，可以咨询**主题专家（SME）**，以深入了解预期结果。这对于具有专业知识的行业（例如金融或医疗保健）尤其重要。
  在 **[自动化测试](/zh-cn/wiki/automated-testing/)** 中，[预期结果](/zh-cn/wiki/expected-result/) 必须精确且明确，因为自动化脚本使用它来断言系统响应的正确性。 [测试自动化](/zh-cn/wiki/test-automation/) 工程师负责将这些[预期结果](/zh-cn/wiki/expected-result/) 编码到[测试脚本](/zh-cn/wiki/test-script/) 中，确保它们符合[测试用例](/zh-cn/wiki/test-case/) 规范。
...
如果是这样，怎么办？
  是的，**[预期结果](/zh-cn/wiki/expected-result/)** 可能会在测试过程中发生变化。发生这种情况的原因有多种：
...
在[测试自动化](/zh-cn/wiki/test-automation/) 中，将**[预期结果](/zh-cn/wiki/expected-result/)** 与**[实际结果](/zh-cn/wiki/actual-result/)** 进行比较是验证[测试用例](/zh-cn/wiki/test-case/) 结果的关键步骤。这种比较通常在[测试脚本](/zh-cn/wiki/test-script/) 内自动进行。其操作方法如下：
...
为了比较 **[预期结果](/zh-cn/wiki/expected-result/)** 与 [测试自动化](/zh-cn/wiki/test-automation/) 中的 **[实际结果](/zh-cn/wiki/actual-result/)**，使用了各种工具和技术：
...
当 **[预期结果](/zh-cn/wiki/expected-result/)** 与 **[实际结果](/zh-cn/wiki/actual-result/)** 不匹配时，表示存在**差异**，该差异可能是由于各种原因造成的，例如代码中的缺陷、[测试用例](/zh-cn/wiki/test-case/) 中的错误或对需求的误解。这种不匹配会触发以下操作：
...
3. **沟通**​​：将问题通知相关利益相关者，例如开发人员和产品所有者，以便采取进一步行动。
4. **解决**：开发团队解决缺陷，解决后，重新执行测试以验证修复。
5. **[测试用例](/zh-cn/wiki/test-case/) 审核**：如果差异是由于测试用例不正确造成的，请更新测试用例以符合正确的预期行为。
...
定义“[预期结果](/zh-cn/wiki/expected-result/)”的最佳实践包括：
...
在 [测试自动化](/zh-cn/wiki/test-automation/) 中定义“[预期结果](/zh-cn/wiki/expected-result/)”时，请避免以下常见错误：
...
可以通过多种方法向测试团队有效传达“[预期结果](/zh-cn/wiki/expected-result/)”：
