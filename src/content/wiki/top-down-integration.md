...
5. 测试集成的模块并重复该过程，直到所有模块都集成并测试完毕。
  当软件项目具有清晰的自上而下的层次结构并且关键功能的早期验证很重要时，[自上而下的集成](/zh-cn/wiki/top-down-integration/) 特别有用。它可以尽早演示软件的功能，并可以促进用户或利益相关者的早期反馈。然而，它对于单独测试较低级别的组件可能不那么有效，这可能导致将这些组件中的某些缺陷的发现推迟到集成过程的后期阶段。
...
- **持续反馈**：监控测试结果和系统行为的机制，以提供对集成问题的即时洞察。
  通过关注这些组件，[测试自动化](/zh-cn/wiki/test-automation/) 工程师可以有效地实施[自上而下的集成](/zh-cn/wiki/top-down-integration/)，确保更高级别的功能指导集成过程，并确保系统架构在开发周期的早期得到验证。
...
7. **最终测试**：集成所有模块后，进行最后一轮彻底测试以验证整个系统。
  在整个过程中，使用**自动化[测试脚本](/zh-cn/wiki/test-script/)**来验证模块交互和功能，确保可重复性和效率。请记住保留每个步骤的清晰**文档**，以便可追溯和将来参考。
...
- **[测试工具](/zh-cn/wiki/test-harness/)** ：软件和测试数据的集合，配置为通过在不同条件下运行程序单元来测试程序单元。
...
[自上而下的集成](/zh-cn/wiki/top-down-integration/) 和[自下而上的集成](/zh-cn/wiki/bottom-up-integration/) 是[软件测试](/zh-cn/wiki/software-testing/) 的两种相反的方法。
  **[自上而下的集成](/zh-cn/wiki/top-down-integration/)** 从测试顶级模块（通常是用户界面或高级逻辑）开始，并逐步集成较低级别的模块。存根或虚拟模块用于模拟尚未集成或开发的较低级别模块的行为。
  另一方面，**[自下而上的集成](/zh-cn/wiki/bottom-up-integration/)** 从最低级别模块的集成开始，例如实用程序函数或[database](/zh-cn/wiki/database/) 交互，然后向上延伸到用户界面。驱动程序是临时代码模块，用于模拟尚未集成的高级模块。
...
- **持续集成 (CI) 工具**，例如 *Jenkins*、*Travis CI* 或 *GitLab CI*，可以自动化测试过程，确保定期运行 [自上而下的集成](/zh-cn/wiki/top-down-integration/) 测试并及时报告结果。
  这些工具有助于以[自上而下的集成](/zh-cn/wiki/top-down-integration/) 方法自动化测试过程，确保在开发周期的早期测试更高级别的模块，并确保正确模拟依赖关系，直到实际实现可用。
...
2. **内容管理系统 (CMS)**：对于具有分层架构的 CMS，开发人员可能首先将用户界面与内容交付应用程序集成，然后与内容管理和 [database](/zh-cn/wiki/database/) 服务进行较低级别的集成。
...
5. **汽车软件系统**：在车辆软件中，[自上而下的集成](/zh-cn/wiki/top-down-integration/) 可能会从信息娱乐系统的用户界面与控制逻辑的集成开始，然后再与较低级别的硬件接口和传感器集成。
