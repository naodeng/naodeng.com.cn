---
title: "视觉回归测试 ｜ Visual Regression Testing"
description: "视觉回归测试 (Visual Regression Testing) 是一种 质量保证 过程，涉及捕获网页或应用程序屏幕的屏幕截图，并将其与基准图像进行比较以检测更改。这种比较通常是逐像素的，突出显示功能测试可能无法捕捉到的视觉差异。关键步骤包括：1. 建立基准：捕获 UI 的屏幕截图，作为未来测试的参考…"
section: "V"
related:
  - screenshot-testing
  - regression-testing
...
- **自动化目视检查：**减少手动检查 UI 元素的需要。

- **快速反馈：**提供有关代码更改对 UI 的影响的即时洞察。
...
- **维护开销：**基线图像需要通过有意的 UI 更改进行更新。

- **有限的背景：**识别视觉变化但不解释原因。

- **动态内容问题：**难以处理在测试之间改变外观的动态内容。 为了减轻缺点，请考虑以下策略：

- **选择性测试：**专注于关键视觉区域以减少噪音。

- **阈值调整：**设置容差级别以最大限度地减少误报。

- **高效存储：**使用图像压缩和重复数据删除来管理资源。

- **定期更新：**保持基线图像最新以反映有意的更改。

- **动态内容处理：**在测试过程中使用策略来稳定动态元素。
...
3. **更新基线**：对于有效的更改，更新基线图像以反映新的预期状态。 使用工具的报告功能来简化流程，通常提供仪表板或更改摘要。自动执行已知变更的审批流程，以关注意外差异。
