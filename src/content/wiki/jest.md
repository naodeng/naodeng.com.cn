...
[Jest](/zh-cn/wiki/jest/) 是一个 **JavaScript 测试框架**，旨在确保任何 JavaScript 代码库的正确性。它允许开发人员使用 [API](/zh-cn/wiki/api/) 编写测试，鼓励良好的测试实践，并且通常用于前端和后端 JavaScript 应用程序。
  使用[Jest](/zh-cn/wiki/jest/)，您可以执行**单元测试**来验证各个功能或模块，**集成测试**以确保应用程序的不同部分按预期协同工作，以及**端到端测试**来测试应用程序的流程。
  [Jest](/zh-cn/wiki/jest/) 与使用 **Babel**、**TypeScript**、**[node.js](/zh-cn/wiki/node-js/)**、**React**、**Angular** 和 **Vue** 的项目完美集成，使其成为各种 JavaScript 项目的多功能选择。它还支持**测试并行化**，其中测试在单独的进程中同时运行，以最大限度地提高性能和速度。
...
与 Mocha、[jasmine](/zh-cn/wiki/jasmine/) 或 AVA 等其他测试框架相比，[Jest](/zh-cn/wiki/jest/) 因其**简单**和**集成功能**而脱颖而出。与 Mocha 不同，Mocha 需要额外的插件来实现模拟、覆盖和快照测试等功能，[Jest](/zh-cn/wiki/jest/) 附带了这些开箱即用的功能。这减少了配置多个库的需要，使 [Jest](/zh-cn/wiki/jest/) 成为更**简化的选择**。
  [Jest](/zh-cn/wiki/jest/) 并行运行测试，与其他一些框架相比，这可以导致**更快的执行时间**。其**监视模式**也针对开发人员体验进行了高度优化，允许自动运行与更改的文件相关的测试。
  与具有类似语法的[jasmine](/zh-cn/wiki/jasmine/) 相比，[Jest](/zh-cn/wiki/jest/) 提供了更**现代且强大的模拟库**。这使得测试 JavaScript 应用程序变得更容易，尤其是那些使用 React 构建的应用程序，其中 [Jest](/zh-cn/wiki/jest/) 通常是推荐的选择，因为它**对 React 测试实用程序的本机支持**。
  虽然 AVA 在测试运行中强调**并发**，但 [Jest](/zh-cn/wiki/jest/) 平衡并行 [测试执行](/zh-cn/wiki/test-execution/) 与**共享上下文**，这对某些类型的 [测试套件](/zh-cn/wiki/test-suite/) 可能是有益的。此外，[Jest](/zh-cn/wiki/jest/) 的**快照测试**功能比其他框架中的类似功能更先进，提供了一种测试组件输出的简单方法。
  对于**异步测试**，[Jest](/zh-cn/wiki/jest/) 支持 async/await、promise 和回调，与其他框架类似，但具有更**统一的语法**和更好的错误处理。
  总体而言，[Jest](/zh-cn/wiki/jest/) 通常因其**开发人员友好的方法**、全面的文档以及与 JavaScript 生态系统的**紧密集成**而受到青睐，特别是在使用 `create-react-app` 或使用 Babel 和 TypeScript 创建的项目中。

#### Jest 的主要特点是什么？

[Jest](/zh-cn/wiki/jest/) 的主要功能包括：
...
要有效使用[Jest](/zh-cn/wiki/jest/)，应满足某些先决条件：
...
要在 [Jest](/zh-cn/wiki/jest/) 中运行测试，请按照下列步骤操作：
...
在[Jest](/zh-cn/wiki/jest/) 中，使用`expect` 函数进行断言，该函数与“匹配器”函数链接以以不同方式测试值。以下是一些常见的断言：
...
在[Jest](/zh-cn/wiki/jest/) 中，`describe` 函数用于**将相关测试组合在一起**。它通过**封装多个测试特定功能或组件的[测试用例](/zh-cn/wiki/test-case/)**来组织[测试套件](/zh-cn/wiki/test-suite/)。这对于可读性和维护特别有用，因为它允许开发人员一目了然地看到哪些测试是相关的，并运行与他们正在处理的代码区域相关的测试子集。
