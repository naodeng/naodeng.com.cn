...
- **复杂[Setup](/zh-cn/wiki/setup/)** ：某些测试场景自动化起来很复杂，可能不值得付出努力。
...
7. **设置[测试环境](/zh-cn/wiki/test-environment/)**：配置运行测试所需的环境，包括浏览器、[databases](/zh-cn/wiki/database/) 和任何其他依赖项。
...
[系统测试](/zh-cn/wiki/system-testing/) 是一个**高级**测试阶段，用于评估完整的集成软件系统以验证其是否满足指定的要求。它在 **[集成测试](/zh-cn/wiki/integration-testing/)** 之后和 **[验收测试](/zh-cn/wiki/acceptance-testing/)** 之前进行，重点关注各种条件下的行为和输出。
  在[系统测试](/zh-cn/wiki/system-testing/)期间，应用程序在与生产非常相似的环境中进行测试，包括**[database](/zh-cn/wiki/database/)交互**、**网络通信**和**服务器交互**。目标是识别只有当组件在系统范围内集成和交互时才会出现的缺陷。
...
端到端 (E2E) 测试是一种在紧密模仿现实世界使用的场景中测试整个应用程序的技术，例如与 [database](/zh-cn/wiki/database/)、网络、硬件和其他应用程序交互。目标是从头到尾验证系统的集成和数据完整性，确保应用程序的所有组件在各种场景下都按预期运行。
...
数据驱动测试 (DDT) 是一种 **[测试自动化](/zh-cn/wiki/test-automation/)** 策略，涉及使用多组输入数据执行一组测试步骤。此方法通过验证各种输入值的应用程序行为来增强[测试覆盖率](/zh-cn/wiki/test-coverage/)，而无需为每个数据集编写多个[测试脚本](/zh-cn/wiki/test-script/)。
  在 DDT 中，测试逻辑与 [测试数据](/zh-cn/wiki/test-data/) 分离，通常存储在外部数据源中，例如 CSV 文件、Excel 电子表格、XML 或 [databases](/zh-cn/wiki/database/)。在[测试执行](/zh-cn/wiki/test-execution/)期间，自动化框架读取数据并将其输入[测试用例](/zh-cn/wiki/test-case/)。
  这是伪代码的简化示例：
