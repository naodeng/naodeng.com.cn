...
- **[非功能测试](/zh-cn/wiki/non-functional-testing/)** ：非常适合难以手动执行的性能、负载和压力测试。
  [自动化测试](/zh-cn/wiki/automated-testing/) 的缺点：
...
- **[用例](/zh-cn/wiki/use-case/)** ：手动测试通常更适合探索性、可用性和临时测试。自动化测试非常适合回归、负载和性能测试等。
...
- **[安全测试](/zh-cn/wiki/security-testing/)**：自动脚本探测应用程序的漏洞，确保软件免受潜在攻击。
  这些技术可以组合和定制，以满足特定的项目要求，确保强大而高效的[自动化测试](/zh-cn/wiki/automated-testing/)流程。
...
5. **存储[测试脚本](/zh-cn/wiki/test-script/)**
...
9. **Manage [测试数据](/zh-cn/wiki/test-data/)**
    和依赖关系，以确保测试运行之间的一致性。
...
9. **管理[测试数据](/zh-cn/wiki/test-data/)**
    和依赖关系，以确保测试运行之间的一致性。
...
11. **与 CI/CD 集成**：作为 CI/CD 管道的一部分自动执行[测试用例](/zh-cn/wiki/test-case/)，以确保每次构建或发布时持续验证 AUT。
  通过遵循这些步骤，[测试自动化](/zh-cn/wiki/test-automation/) 工程师可以创建健壮、可靠且有效的自动化[测试用例](/zh-cn/wiki/test-case/)，从而有助于提高软件产品的整体质量。
...
- **可靠**：它们产生一致的结果并优雅地处理异常。
  脚本通常被组织成 [测试套件](/zh-cn/wiki/test-suite/) 以获得更好的可管理性，并且可以单独运行或作为更大的测试运行的一部分运行。它们对于持续集成和交付管道至关重要，可以对软件构建进行频繁的自动验证。
...
- **[代码覆盖率](/zh-cn/wiki/code-coverage/) 分析**：使用工具测量测试执行的代码范围，旨在获得高覆盖率指标，例如语句、分支和路径覆盖率。
  将这些策略纳入您的测试设计过程中，以创建全面的[测试套件](/zh-cn/wiki/test-suite/)。定期审查和更新[测试用例](/zh-cn/wiki/test-case/)以适应应用程序及其使用模式的变化。
...
- **弃用策略**：制定明确的策略来弃用和删除过时的测试，以避免混乱和混乱。
  通过应用这些策略，[测试自动化](/zh-cn/wiki/test-automation/) 工程师可以确保他们的[测试套件](/zh-cn/wiki/test-suite/) 随着时间的推移保持稳健、相关和可靠，从而在软件开发生命周期中提供持续的价值。
...
- **[兼容性测试](/zh-cn/wiki/compatibility-testing/)** ：确认软件可以在不同的设备、浏览器和操作系统上运行。
  自动化[系统测试](/zh-cn/wiki/system-testing/)可以显着**减少执行重复但必要的检查所需的时间**，从而允许更频繁和彻底的测试周期。它对于 **[回归测试](/zh-cn/wiki/regression-testing/)** 特别有用，可确保新更改不会对现有功能产生不利影响。但是，它可能无法完全取代[手动测试](/zh-cn/wiki/manual-testing/)，特别是对于探索性、可用性和临时测试场景。
...
- **重构信心**：自动化测试允许开发人员重构代码，并确保现有功能保持不变。
  TDD 通过确保从开发过程的一开始就考虑测试而不是事后才考虑测试，从而补充了其他[自动化测试](/zh-cn/wiki/automated-testing/) 策略。它鼓励一种可以带来更高质量软件的测试规则，并且非常适合敏捷和持续集成/持续部署 (CI/CD) 工作流程。
