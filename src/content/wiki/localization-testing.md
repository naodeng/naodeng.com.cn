...
- [国际化测试](/zh-cn/wiki/internationalization-testing/)
...
[本地化测试](/zh-cn/wiki/localization-testing/) 是一个确保软件应用程序适合特定区域或语言的过程，不仅验证文本翻译，还验证文化差异、货币、日期格式和其他特定区域元素的适应性。它是全球化测试的一个子集，一次专注于一个特定的区域设置。
  要有效地执行[本地化测试](/zh-cn/wiki/localization-testing/)，请考虑目标区域设置中应用程序使用的**上下文**。这包括语言准确性、文化适宜性以及与当地标准的技术一致性。 **伪本地化** 可以作为模拟翻译和检测 UI 布局或字符编码潜在问题的初步步骤。
  [本地化测试](/zh-cn/wiki/localization-testing/) 中的自动化可以使用支持国际化库和特定于语言环境的数据集的工具来实现。可以设计脚本来切换区域设置并相应地验证应用程序的行为。例如：
...
```
将 **区域特定的 [测试用例](/zh-cn/wiki/test-case/)** 纳入您的自动化套件中，以确保覆盖不同的语言和地区。定期更新这些案例以反映应用程序和目标区域设置的变化。
  请记住，[本地化测试](/zh-cn/wiki/localization-testing/) 不仅仅涉及翻译，还涉及确保应用程序引起当地受众的共鸣并遵守他们的期望和标准。这是向全球用户提供本土化产品的关键一步。

#### 为什么本地化测试很重要？

[本地化测试](/zh-cn/wiki/localization-testing/) 对于确保软件产品**适当地适应**特定目标市场、考虑语言、文化规范和其他区域细节至关重要。它有助于识别**特定于区域设置的问题**，例如内容截断、不正确的日期和时间格式、货币问题等，否则可能会导致**误解**或**用户沮丧**。
  通过执行[本地化测试](/zh-cn/wiki/localization-testing/)，您可以确保产品**满足本地用户的期望**，这对于**全球市场的成功**至关重要。它不仅涉及翻译[验证 ](/zh-cn/wiki/verification/)，还涉及检查**文化适当性**、**法律要求**和**当地标准合规性**。
  忽视[本地化测试](/zh-cn/wiki/localization-testing/)可能会导致**用户疏远**、**品牌损害**以及潜在的**法律后果**。对于旨在**跨国存在**的产品来说，这是质量保证流程中的关键步骤。
  为了确保彻底性，结合**自动化和[手动测试](/zh-cn/wiki/manual-testing/)工作**，使用**[本地化测试](/zh-cn/wiki/localization-testing/)工具**，并让**母语人士**参与语言和文化见解。自动化对于重复性任务特别有用，例如跨不同区域检查 UI 元素，但 [手动测试](/zh-cn/wiki/manual-testing/) 对于理解文化相关性的**细微差别**至关重要。
  对于从右向左阅读的语言，需要特别注意**UI布局**、**文本对齐**和**输入字段方向性**。通过使用**模拟器**、**众包测试**和**特定于脚本的测试工具**可以缓解**对母语人士的访问受限**或**复杂的脚本处理**等挑战。
  纳入最佳实践，例如**本地化团队的早期参与**、**持续本地化**以及翻译记忆库的**定期更新**。成功的[本地化测试](/zh-cn/wiki/localization-testing/)通常涉及**协作方法**，利用本地用户的**反馈循环**来迭代地完善产品。

#### 本地化测试需要考虑哪些关键要素？

在考虑 **[本地化测试](/zh-cn/wiki/localization-testing/)** 中的关键元素时，请重点关注：
...
- **自动化[测试用例](/zh-cn/wiki/test-case/)** ：开发可以轻松适应不同区域设置的自动化测试，重点关注上述元素。 通过解决这些要素，您可以提高软件在不同市场的质量和相关性。

#### 本地化测试如何改善整体用户体验？

[本地化测试](/zh-cn/wiki/localization-testing/) 通过确保软件感觉是为来自不同地区的用户**原生设计**，从而增强整体用户体验。它不仅仅是翻译，还解决文化差异、日期、货币和地址的本地格式以及遵守当地法规。这种对细节的关注使用户在使用产品时更加舒适和自信，从而提高**满意度**和**参与度**。
  ...
  总体而言，[本地化测试](/zh-cn/wiki/localization-testing/) 是交付一款能够与全球用户产生共鸣的精美产品的关键组成部分，确保该软件不仅能够在不同的地区正常运行，而且让使用它的人**感觉良好**。

#### 不进行本地化测试的潜在后果是什么？

忽视[本地化测试](/zh-cn/wiki/localization-testing/)可能会导致多种不良后果：
...
- **延迟发布**：未识别的本地化问题可能会导致产品发布延迟，因为它们可能需要在开发周期后期进行大量返工。 总之，缺乏彻底的[本地化测试](/zh-cn/wiki/localization-testing/) 可能会严重影响软件产品的成功和全球影响力。

### 流程和技术

#### 本地化测试涉及哪些步骤？

[本地化测试](/zh-cn/wiki/localization-testing/) 涉及多个步骤，以确保软件在不同区域设置中按预期运行。这是一个简洁的细分：
...
4. **[功能测试](/zh-cn/wiki/functional-testing/)**：验证应用程序在每个区域设置中是否正常运行。这包括输入字段验证、排序、搜索以及可能因地区而异的其他功能。
5. **UI 和布局测试**：检查 UI 是否可以容纳翻译后的文本而不会截断或重叠，并且布局更改（如果有）是否适合目标区域设置。
6. **文化正确性**：评估内容、符号、颜色和图像的文化适当性，以避免冒犯用户。
7. **[兼容性测试](/zh-cn/wiki/compatibility-testing/)**：确保应用程序与本地化操作系统、浏览器和设备兼容。
8. **[性能测试](/zh-cn/wiki/performance-testing/)**：评估应用程序的性能，考虑处理多个区域设置的额外负载。
9. **[回归测试](/zh-cn/wiki/regression-testing/)**：进行回归测试以确认新的更新或修复没有引入新的本地化问题。
10. **审查和反馈**：收集母语人士的反馈并解决发现的任何语言或文化问题。
11. **最终[验证 ](/zh-cn/wiki/verification/)**：执行最后一轮测试，以确保所有问题均已解决，并且应用程序已准备好在目标市场发布。

#### 本地化测试中常用哪些技术？

**[本地化测试](/zh-cn/wiki/localization-testing/)** 中的常用技术包括：
...
- **[功能测试](/zh-cn/wiki/functional-testing/)**：验证应用程序在本地化版本中是否正常运行，包括特定于区域设置的输入方法、排序和数据格式。
- **合规性测试**：确保软件符合当地法律和文化规范，包括内容法规、数据隐私法和交易要求。
- **自动化脚本国际化**：脚本旨在处理多种语言和区域设置，通常使用外部数据文件进行输入，以便在不同的 [测试用例](/zh-cn/wiki/test-case/) 之间轻松切换。
- **区域特定的[测试用例](/zh-cn/wiki/test-case/)**：[测试用例](/zh-cn/wiki/test-case/) 是为解决区域设置特定的场景而定制的，例如日期和时间格式、货币和地址结构。
- **持续本地化**：将本地化集成到持续开发和测试周期中，允许在添加新内容时进行持续测试和更新。 这些技术通常结合起来创建一个全面的[本地化测试](/zh-cn/wiki/localization-testing/)策略，以确保该软件受到不同地区用户的好评。

#### 本地化测试与其他类型的软件测试有何不同？

[本地化测试](/zh-cn/wiki/localization-testing/) 与其他类型的[软件测试](/zh-cn/wiki/software-testing/) 的不同之处在于它**特别关注不同目标市场的文化和语言准确性**。与[功能测试](/zh-cn/wiki/functional-testing/)（确保软件正确运行）或[性能测试](/zh-cn/wiki/performance-testing/)（衡量软件在各种条件下的响应能力和稳定性）不同，[本地化测试](/zh-cn/wiki/localization-testing/) 验证软件是否适合不同地理区域的用户。
  此类测试不仅涉及检查文本翻译，还涉及**日期、货币和数值的格式**，以及确保图像、颜色和内容在文化上适当且不会造成冒犯。它还包括验证软件是否可以处理多种语言的输入和显示，特别是那些具有独特字符集或方向性的语言，例如中文或阿拉伯语。
  [本地化测试](/zh-cn/wiki/localization-testing/) 的独特之处在于它需要**语言技能和技术理解的结合**。测试人员必须熟悉目标地区的当地文化和语言细微差别，以及所测试软件的技术方面。
  另一个区别是[本地化测试](/zh-cn/wiki/localization-testing/) 通常需要**额外的工具和技术**来管理多语言[测试用例](/zh-cn/wiki/test-case/) 和数据。例如，字符编码问题特定于[本地化测试](/zh-cn/wiki/localization-testing/)，并且需要可以处理不同文本编码的工具。
  总之，[本地化测试](/zh-cn/wiki/localization-testing/) 是一种专门的测试形式，可确保软件产品完全适应并可以在特定区域使用，而不仅仅是文本翻译，还涵盖该测试形式所特有的文化、功能和技术方面。

#### 如何确保本地化测试的彻底性？

为了确保 [本地化测试](/zh-cn/wiki/localization-testing/) 的彻底性，请考虑以下策略：
...
- **自定义脚本**：使用 **Python** 或 **JavaScript** 等编程语言编写自定义脚本，以自动执行本地化内容的[验证 ](/zh-cn/wiki/verification/)。 这些工具通常集成到 CI/CD 管道中，以确保在整个开发生命周期中持续[本地化测试](/zh-cn/wiki/localization-testing/)。

### 挑战和解决方案

#### 本地化测试中遇到哪些常见挑战？

[本地化测试](/zh-cn/wiki/localization-testing/) 确保软件适用于特定区域或语言。常见的挑战包括：
...
- **自动[测试脚本](/zh-cn/wiki/test-script/) 适应**：脚本可能需要进行重大调整才能处理本地化版本，包括定位器和验证点的更改。
...
缓解[本地化测试](/zh-cn/wiki/localization-testing/) 中的挑战涉及战略规划和资源的有效利用。以下是一些方法：
...
- **持续集成**：将 [本地化测试](/zh-cn/wiki/localization-testing/) 集成到您的 CI/CD 管道中，以便尽早且经常发现问题。
- **众包测试**：考虑众包测试平台以从不同的地区获得快速反馈。
- **保持更新**：及时了解语言变化和区域趋势，以确保您的测试保持相关性。
- **文档**：维护本地化要求和[测试用例](/zh-cn/wiki/test-case/)的清晰文档，以确保清晰度和可重复性。 通过采用这些策略，您可以降低本地化问题的风险，并确保为所有目标市场提供高质量的用户体验。
...
- **众包测试**：考虑众包测试平台以从不同的地区获得快速反馈。
- **保持更新**：及时了解语言变化和区域趋势，以确保您的测试保持相关性。
- **文档**：维护本地化要求和[测试用例](/zh-cn/wiki/test-case/)的清晰文档，以确保清晰度和可重复性。

#### 本地化测试的最佳实践有哪些？

[本地化测试](/zh-cn/wiki/localization-testing/) 的最佳实践包括：
...
[本地化测试](/zh-cn/wiki/localization-testing/) 中的自动化可以显着简化跨不同语言和地区验证软件功能的过程。自动化测试可以设计为**验证 UI 元素**，例如按钮、菜单和对话框，确保文本正确显示，没有重叠或截断，并且针对目标受众进行了正确本地化。
  使用**参数化测试**，自动化框架可以迭代各种本地化数据集，检查每种支持语言的一致性和正确性。此方法允许重用[测试用例](/zh-cn/wiki/test-case/)，其中仅输入数据根据所测试的区域设置而更改。
  ...
  **伪本地化**可以自动化，以确保软件可以处理不同长度的翻译字符串，并且对于非标准字符仍然有效。该技术涉及用更改后的版本替换原始文本，其中包括扩展字符和符号以模仿不同的语言。
  ...
  为了处理[本地化测试](/zh-cn/wiki/localization-testing/)的复杂性，自动化工程师经常将**[本地化测试](/zh-cn/wiki/localization-testing/)工具**与其自动化框架集成。这些工具可以自动检测缺失的翻译、占位符的错误使用以及其他常见的本地化问题。
  通过自动执行重复且耗时的任务，[测试自动化](/zh-cn/wiki/test-automation/) 使团队能够专注于更复杂的本地化挑战，最终确保为国际用户提供更高质量的产品。

#### 您如何处理从右向左阅读的语言（如阿拉伯语和希伯来语）的本地化测试？

由于独特的布局和文本方向，处理从右向左 (RTL) 阅读的语言（例如阿拉伯语和希伯来语）的 [本地化测试](/zh-cn/wiki/localization-testing/) 需要特别注意细节。以下是一些策略：
...
- **自动化[测试脚本](/zh-cn/wiki/test-script/)** ：调整自动化测试脚本以适应 RTL 输入并相应地验证 UI。 RTL 语言的 [测试脚本](/zh-cn/wiki/test-script/) 片段示例：
...
```
请记住**与母语人士进行验证**以确保本地化的准确性和适当性，并**在添加新功能或修改现有功能时定期更新您的[测试用例](/zh-cn/wiki/test-case/)**。

### 实际应用

#### 您能提供成功的本地化测试示例吗？

成功的[本地化测试](/zh-cn/wiki/localization-testing/) 可确保软件在文化和语言上适合目标受众。以下是示例：
  **Netflix** 在本地化方面表现出色，为不同地区量身定制内容和字幕，导致全球订阅量大幅增长。
  **Microsoft Office** 套件提供了 [本地化测试](/zh-cn/wiki/localization-testing/) 的绝佳示例，其用户界面、帮助内容和模板适合不同语言和地区，保持了功能和用户体验的一致性。
  ...
  **Uber** 通过集成本地地图、支付方式以及对从右到左语言的支持来本地化其应用程序，这对于其拓展新市场至关重要。
  要自动化[本地化测试](/zh-cn/wiki/localization-testing/)，请考虑使用以下工具：
...
糟糕的 [本地化测试](/zh-cn/wiki/localization-testing/) 可能会导致一些现实世界的后果：
...
- **产品延迟**：在开发周期后期发现本地化问题可能会导致产品发布延迟，影响上市时间和潜在收入。 有效的[本地化测试](/zh-cn/wiki/localization-testing/)对于避免这些后果并确保产品在不同地区和文化中受到好评至关重要。

#### 本地化测试如何应用于移动应用程序？

移动应用程序的 [本地化测试](/zh-cn/wiki/localization-testing/) 可确保应用程序在不同的区域设置中正常运行。这涉及验证**文本翻译**、**日期和时间格式**、**货币**以及其他特定于语言环境的元素。考虑到文本扩展或收缩可能会影响 UI 元素，它还会检查各种语言设置下的正确布局和功能。
...
此代码片段将西班牙的区域设置设置为西班牙语，这可以是检查应用程序的西班牙语本地化的大型自动化测试的一部分。
  ...
  总之，移动应用的 [本地化测试](/zh-cn/wiki/localization-testing/) 旨在通过自动化本地化资源和 UI 元素的 [验证 ](/zh-cn/wiki/verification/) 来确保所有用户（无论其语言或地区如何）获得无缝的用户体验。
...
此代码片段将浏览器语言设置为西班牙语，这对于测试 Web 应用程序的西班牙语本地化非常有用。
  ...
  通过将 [本地化测试](/zh-cn/wiki/localization-testing/) 合并到自动化套件中，您可以确保 Web 应用程序为所有用户提供无缝且与文化相关的体验，无论他们位于何处。

#### 本地化测试如何应用于桌面应用程序？

用于桌面应用程序的[本地化测试](/zh-cn/wiki/localization-testing/) 可确保软件针对特定区域或语言进行正确调整。这不仅涉及验证文本的翻译，还涉及文化规范、货币格式、日期和时间格式以及其他特定于语言环境的元素的适当性。
...
- **功能**在不同的区域设置中保持一致。 以下示例说明了如何使用伪代码在桌面应用程序中自动执行简单的区域设置更改和[验证 ](/zh-cn/wiki/verification/)：
...
对于每个受支持的区域设置，您可以使用适当的参数调用`testLocalization`。这可确保应用程序按照不同地区的用户的预期运行。
  请记住，**自动化**是有效覆盖多种区域设置和语言组合的关键。然而，至关重要的是用 **[手动测试](/zh-cn/wiki/manual-testing/)** 来补充自动化检查，以捕获自动化测试可能遗漏的上下文和文化适当性的细微差别。
