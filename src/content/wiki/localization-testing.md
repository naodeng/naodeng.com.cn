...
[本地化测试](/zh-cn/wiki/localization-testing/) 对于确保软件产品**适当地适应**特定目标市场、考虑语言、文化规范和其他区域细节至关重要。它有助于识别**特定于区域设置的问题**，例如内容截断、不正确的日期和时间格式、货币问题等，否则可能会导致**误解**或**用户沮丧**。
  通过执行[本地化测试](/zh-cn/wiki/localization-testing/)，您可以确保产品**满足本地用户的期望**，这对于**全球市场的成功**至关重要。它不仅涉及翻译[verification](/zh-cn/wiki/verification/)，还涉及检查**文化适当性**、**法律要求**和**当地标准合规性**。
...
11. **最终[Verification](/zh-cn/wiki/verification/)**：执行最后一轮测试，以确保所有问题均已解决，并且应用程序已准备好在目标市场发布。
1. **准备**：定义本地化范围，包括语言和区域。收集所有必要的资源，例如本地化字符串、区域数据格式和文化差异。
2. **伪本地化**：实施伪本地化以在应用实际翻译之前检测 UI 布局和字符编码的潜在问题。
3. **翻译**：将翻译集成到应用程序中。确保所有 UI 元素、文档和帮助文件均已翻译。
4. **[功能测试](/zh-cn/wiki/functional-testing/)**：验证应用程序在每个区域设置中是否正常运行。这包括输入字段验证、排序、搜索以及可能因地区而异的其他功能。
...
此代码片段将西班牙的区域设置设置为西班牙语，这可以是检查应用程序的西班牙语本地化的大型自动化测试的一部分。
  ...
  总之，移动应用的 [本地化测试](/zh-cn/wiki/localization-testing/) 旨在通过自动化本地化资源和 UI 元素的 [verification](/zh-cn/wiki/verification/) 来确保所有用户（无论其语言或地区如何）获得无缝的用户体验。
...
- **功能**
    在不同的区域设置中保持一致。
  以下示例说明了如何使用伪代码在桌面应用程序中自动执行简单的区域设置更改和[verification](/zh-cn/wiki/verification/)：
