...
6. **[静态测试](https://inaodeng.com/zh-cn/wiki/static-testing/)**：[V 型](https://inaodeng.com/zh-cn/wiki/v-model/) 推广[静态测试](https://inaodeng.com/zh-cn/wiki/static-testing/) 方法，例如评论和[[检查](https://inaodeng.com/zh-cn/wiki/inspection/)s](https://inaodeng.com/zh-cn/wiki/inspection/)，以在不执行代码的情况下捕获缺陷。
...
- **缺陷预防：**由于开发阶段和相应的测试阶段之间的密切关系，可以及早发现和预防缺陷。
...
- **后期整合：**系统集成发生在周期的后期，这可能导致在项目后期发现严重的集成问题。

- **不适合复杂项目：**对于从一开始就没有清楚理解需求的项目，效率较低。
...
- **纪律严明：**在编码开始之前强制采用严格的方法进行需求规范和设计。
...
- **无迭代开发：**缺乏敏捷方法中的迭代过程，使其难以适应不断变化的需求或不断发展的技术。
...
- **沟通差距**：团队之间可能会出现沟通不畅的情况。福斯特 **定期跨团队会议**和 **清晰的文档**弥补差距。
...
- **被忽视的缺陷**：一些缺陷可能会被遗漏。雇用 **同行评审**和 **静态分析工具**及早发现问题。
