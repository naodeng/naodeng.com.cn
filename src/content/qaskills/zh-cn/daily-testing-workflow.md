...
- 何时使用：你需要做 日常任务协同和阻塞管理，而不是零散执行测试。
- 怎么使用：先定范围和时间，再按阶段推进，最后输出状态、风险和下一步动作。


## 使用说明
1. 先完成 `daily-testing-workflow` 的安装和启用（直接用本页安装命令）。
2. 提需求时先给必要上下文：范围、环境、时间、期望输出格式。
3. 用明确指令触发，例如：“使用 `daily-testing-workflow` 给我生成这次版本/迭代/日常测试流程。”
