...
Compare two checkout versions to decide whether a latency regression is real, how large it is, and whether it blocks release. The useful record is the decision trail: which version supplied the evidence, which finding has runtime proof, and which item is still open. Write that down early—test design, release review, and the next handoff all depend on it.
