...
Investigate checkout-success and latency anomalies by validating the window, metric definition, impact scope, and candidate causes. The useful record is the decision trail: which version supplied the evidence, which finding has runtime proof, and which item is still open. Write that down early—test design, release review, and the next handoff all depend on it.
