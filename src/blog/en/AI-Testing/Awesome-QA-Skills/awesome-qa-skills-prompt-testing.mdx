...
Test an order-assistant prompt for instruction following, factual citation, boundary refusal, multi-turn consistency, and version regression. The useful record is the decision trail: which version supplied the evidence, which finding has runtime proof, and which item is still open. Write that down early—test design, release review, and the next handoff all depend on it.
