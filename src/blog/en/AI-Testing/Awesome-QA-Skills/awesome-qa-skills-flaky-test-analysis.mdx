...
Analyze intermittent UI regression failures and separate product defects, synchronization faults, unstable environments, and dirty data. The useful record is the decision trail: which version supplied the evidence, which finding has runtime proof, and which item is still open. Write that down early—test design, release review, and the next handoff all depend on it.
