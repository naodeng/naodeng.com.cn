---
title: "根因分析：从现象到假设，找到真正可验证的问题根因"
...
根因分析的目标不是找到一个方便归责的人或组件，而是解释为何防线没有阻止影响扩大。结论必须能被时间线、实验和证据支撑。
...
根因分析 面向需要把测试判断讲清楚、交接出去的工作。它把项目材料、判断依据和下一步动作放在同一条线上——读者可以先知道该检查什么，再决定怎么执行与复核。本文用一个具体场景展开，并保留需要人工确认的边界。
