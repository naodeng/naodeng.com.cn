...
export const DSH_QA_RELEASE_FACTS: DshQaReleaseFact[] = [
  { version: "v0.6.0", publishedAt: "2026-09-24", url: "https://github.com/naodeng/dsh-qa/releases/tag/v0.6.0" },
...
export const DSH_QA_SITE: Record<Lang, DshQaSiteCopy> = {
  en: {
    ...
    releases: releases([
      { summary: "Added controlled Native QA Execution and an Action Desk for evidence-bound execution work.", changes: ["Added Native QA Execution with controlled Host execution profiles, preview and confirmation, start, cancel, retry, timeout, and evidence archiving.", "Added an Action Desk backed by the canonical Action Queue, preserving legacy dashboard reminders while adding bilingual states, stable ordering, and SSE refresh.", "Persisted Host Execution evidence boundaries and lifecycle transitions separately from local TestRun events, preventing evidence-free results from becoming quality conclusions.", "Fixed optional hostAdapters context access in the DSH v0.1.7-rc.1 host and hardened Action Desk revision, duplicate-start, retry, cancellation, and error paths.", "Verified 213 unit/API tests, 37 Chromium E2E tests, npm pack --dry-run, and local dsh-v0.1.7-rc.1 host compatibility acceptance."] },
  ...
  "zh-cn": {
    ...
    releases: releases([
      { summary: "新增受控的 Native QA Execution 与 Action Desk，让执行工作具备可追溯证据边界。", changes: ["新增 Native QA Execution，为 Host 执行配置、预览确认、启动、取消、重试、超时和证据归档建立受控链路。", "新增由 canonical Action Queue 驱动的 Action Desk，保留首页提醒兼容投影，并支持双语状态、稳定排序和 SSE 刷新。", "持久化 Host Execution 证据边界与生命周期，将其与本地 TestRun 事件分离，阻止无证据的执行结果进入质量结论。", "修复 DSH v0.1.7-rc.1 宿主中可选 hostAdapters 上下文访问，并强化 Action Desk 的 revision、重复启动、重试、取消和错误路径。", "验证通过 213 个单元/API 测试、37 个 Chromium E2E 测试、npm pack --dry-run，以及本地 dsh-v0.1.7-rc.1 宿主兼容验收。"] },
