...
for (const locale of ["en", "zh-cn"] as const) {
  ...
  test(`${locale} dsh-qa presents the complete product page with the latest release expanded`, async ({ page }) => {
    ...
    await expect(page.locator("[data-dsh-qa-latest-release]")).toContainText("v0.6.0");
    await expect(page.locator("[data-dsh-qa-latest-release]")).toHaveAttribute("href", "https://github.com/naodeng/dsh-qa/releases/tag/v0.6.0");
    ...
    await expect(page.locator("[data-dsh-current-release]")).toContainText("v0.6.0");
    await expect(page.locator("[data-dsh-release-history]")).not.toHaveAttribute("open", "");
    await expect(page.locator("[data-release-note]")).toHaveCount(18);
  });
